...
- `과목 초기화`는 현재 과목 시간을 처음부터 다시 세팅하고, 전체 남은 시간도 현재 과목 이후 기준으로 다시 계산합니다. 현재 과목의 문항별 시간 기록만 지우고 OMR도 해당 과목 1번으로 돌려놓습니다.
- `전체 초기화`는 전체 타이머 흐름을 `1과목 / 75:00` 기준으로 다시 세팅하고, 문항 시간 기록과 과목 잠금을 모두 초기화합니다. 답안/정답은 지우지 않습니다.
- 스테이징 로컬 브라우저에서 `74:49 -> 과목 초기화 -> 75:00`, `2과목 59:57 -> 전체 초기화 -> 1과목 75:00` 흐름까지 실클릭으로 확인했습니다.
- 이후 고급모드 상단 타이머와 좌측 패널 레이아웃도 스테이징 기준으로 운영에 이식했습니다. 재생 버튼이 밀리지 않게 타이머 셸 폭을 재배치하고, 버튼 라벨을 `다음 / 과↺ / 전↺`로 압축했습니다.
- 일반모드 `고급 기능 안내`와 고급모드 `고급 기능` 설명에도 `타이머 초기화` 항목을 추가해 구독 이유가 드러나도록 보강했습니다.
- 운영 코드 기준 최신 커밋은 `aef45ab`입니다.
...
- 이후 일반모드/고급모드의 고급 안내 문구도 조정해, `정오표 일괄입력`을 3번째 핵심 기능으로 올리고 문항 번호 없이도 정답 열 중심 형식은 유연하게 불러올 수 있다는 점을 반영했습니다.
- 운영 코드 기준 최신 커밋은 `a7ec4f7`입니다.
...
## 2026-04-06 고급 비밀번호 오류 메시지 분리
- 운영 기준 커밋은 `5ff5978`입니다.
- 사용자 제보
  - `manual-seed-dgd0392`가 비밀번호처럼 보이고, 실제 비밀번호 `dgd0392`를 넣어도 만료된 것처럼 보인다는 문의
- 사실 확인
  - `manual-seed-dgd0392`는 비밀번호가 아니라 운영 구독 레코드의 `externalId`임
  - 운영 RTDB의 `dgd0392` 구독은 `active`, 만료일 없음 상태로 정상 저장되어 있었음
  - 운영 실페이지에서 `dgd0392` 입력 시 실제 인증은 성공했음
  - 문제의 핵심은 `비밀번호 불일치`와 `만료/비활성`을 한 문장으로 묶어 보여주는 오류 안내 문구였음
- 수정 내용
  - 운영/스테이징 공통 JS에 상세 검증 결과(`active / invalid / expired`)를 반환하는 분기 추가
  - 일반 `고급 안내` 모달과 숨김 `advanced-tools.html` 모두에서 만료/불일치 메시지를 분리
  - 운영/스테이징 진입 HTML의 JS 버전을 `v2026.04.06.1600`으로 올려 캐시 회피 강화
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - `advanced-tools.html` 인라인 스크립트 문법 확인
  - 운영 실페이지에서 `dgd0392` 입력 후 고급 팝업 열기 재현 확인
