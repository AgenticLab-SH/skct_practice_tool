...
- 최근 운영 공개 반영은 `public-clean` 커밋 `c605166` 기준으로 끝났습니다.
...
- 로컬 관리자 진입은 `py scripts/open_local_admin.py` 한 줄로 열 수 있게 정리했고, 종료도 `py scripts/stop_local_admin.py`로 맞췄습니다.
- VS Code에서는 `.vscode/tasks.json` 작업이 같은 Python 스크립트를 바로 실행합니다.
- 로컬 관리자 저장은 Firebase Auth 사용자 확인만으로 끝내지 않고, 필요하면 RTDB 연결을 다시 붙인 뒤 한 번 더 재시도하도록 보강했습니다.
- 운영 RTDB rules에는 `config/.write = auth != null`를 실제 반영해, 로그인된 관리자 기준의 공지/기본값/후원/문구 저장이 다시 정상 동작합니다.
- 로컬 관리자 상단에는 이제 `세션 상태 + 마지막 저장 결과` 배너가 표시되고, 공지 저장뿐 아니라 신청 승인/반려/삭제, 백업 복원, 키 생성 같은 주요 운영 작업도 같은 상태줄로 성공/실패를 바로 남깁니다.
...
- 일반/고급 버튼 내부 설명은 `한 줄 흐름 요약 -> 핵심 조작 -> 접힌 세부 설명` 구조로 다시 맞추고, 같은 문장을 여러 모달에서 반복하지 않도록 역할을 더 강하게 분리했습니다.
- 이 안내 흐름 재정리와 후속 메모장 drag 안정화 수정은 `public-clean` 공개 배포에도 반영되어, 현재 라이브 기준 자산 버전은 `v2026.04.10.2048`입니다.
- 고급 모드 우측 상단 말풍선 버튼은 더 이상 placeholder 경고창이 아니라 `고급 활용` 모달을 바로 여는 빠른 진입점으로 동작합니다.
- `더보기 > 자료 보관함`은 새 탭만 열고 현재 연습 페이지는 그대로 유지되도록 수정했습니다.
- 고급 라이선스가 활성화되면 타이머의 `다음 / 과↺ / 전↺` 버튼 제목과 비활성 상태를 즉시 다시 계산해, 첫 진입 직후에도 현재 상태와 맞는 안내가 보이도록 맞췄습니다.
- VS Code 로컬 관리자 실행 작업은 PowerShell `-Command` 충돌이 나지 않도록 `cmd.exe /c` 기반 process task로 정리했습니다.
- 일반 모드와 고급 모드는 같은 `#notepad` 입력창 로직을 공유하며, 메모장 drag auto-scroll 보조는 edge 근처에서만 돌고 selection이 실제로 멈춘 경우에만 보조 확장을 넣도록 다시 안정화했습니다.
