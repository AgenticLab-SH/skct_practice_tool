...
- 최근 운영 공개 반영은 `public-clean` 커밋 `b7f8bca` 기준으로 끝났습니다.
...
- 이 안내 흐름 재정리와 후속 copy 수정은 `public-clean` 공개 배포에도 반영되어, 현재 라이브 기준 자산 버전은 `v2026.04.10.1645`입니다.
...
- 일반 모드와 고급 모드는 같은 `#notepad` 입력창 로직을 공유하며, 메모장 클릭은 native caret 이동만 남기고 drag auto-scroll 보조는 실제 드래그가 시작됐을 때만 켜지게 조정했습니다.
