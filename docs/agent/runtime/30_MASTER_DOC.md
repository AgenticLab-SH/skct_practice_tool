...
- 최근 운영 공개 반영은 `public-clean` 커밋 `415950f` 기준으로 끝났습니다.
...
- 이 안내 흐름 재정리와 후속 메모장 drag 단순화 수정은 `public-clean` 공개 배포에도 반영되어, 현재 라이브 기준 자산 버전은 `v2026.04.10.2104`입니다.
...
- 일반 모드와 고급 모드는 같은 `#notepad` 입력창 로직을 공유하며, 메모장 drag는 custom 보조 대신 native textarea selection에 맡기고 계산기 포커스 개입만 짧게 막도록 단순화했습니다.
