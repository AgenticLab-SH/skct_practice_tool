...
- 처음 라이브 검증에서 `fetchConfig()`가 실패하는 문제를 확인했고, 원인은 `database.rules.json`에 `config/notice_sidebar` 읽기 허용이 빠져 있던 것이었다.
- 이후 RTDB rules에 `notice_sidebar` 읽기 허용을 추가하고 다시 배포했다.
- 최종 라이브 검증에서 공지 버튼 클릭 시 요청한 문구와 `마지막 업데이트: 2026-04-09`가 정상 표시되는 것을 확인했다.
