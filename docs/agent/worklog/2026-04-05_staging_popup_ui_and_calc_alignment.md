...
- 이후 반복 지시를 줄이기 위해, `일반모드 / 고급모드 / 스테이징 / 운영` 같은 표준 용어를 문서로 고정
- OMR 문항 번호 열 정렬과 선택지 버튼 간격을 더 촘촘하게 정리
- 고급 기능 안내 문구를 일반모드용/고급모드용으로 분리 유지한 상태에서, 더 설명력 있고 구매 유도형 문장으로 정리
- 건너뛴 문제도 정답 입력이 가능한지 로직 확인 후, 실제 동작을 더 명확히 보여주는 안내 문구를 OMR에 추가

## 2026-04-06 지시 용어집 추가
- 새 문서 `docs/SKCT_TOOL_지시_용어집.md` 생성
- `normal 모드 -> 일반모드`, `고급버전 -> 고급모드`, `테스트사이트 -> 스테이징`, `실서버 -> 운영` 기준을 고정
- 단순 용어 치환만이 아니라 `스테이징 반영`, `운영 반영`, `이식`, `기본값 저장`처럼 작업 지시 문장도 같이 표준화

## 2026-04-06 OMR 정렬 보정
- `staging/site/assets/styles/main.css`
  - 문항 번호 영역(`.q-num`) 폭을 고정해서 1자리/2자리 번호가 같은 시작선에 맞도록 조정
  - 선택지 행(`.q-options`)의 간격을 더 좁게 유지하도록 구조 정리
  - 추가 보정으로 번호 텍스트 정렬을 오른쪽에서 왼쪽으로 바꿔 `1.` 앞 여백처럼 보이는 현상을 제거
  - 추가 보정으로 행 gap을 0으로 줄이고 번호 칸 폭도 `18px`까지 타이트하게 줄여 `nn.` 뒤 공백처럼 보이는 영역을 최소화
...
- 기대 효과
  ...
  - `nn.` 앞뒤 공백처럼 보이는 영역이 최소화되어 번호와 선택지가 더 촘촘하게 붙어 보임

## 2026-04-06 운영 이식
- staging에서 승인된 OMR 번호 정렬/간격 보정을 운영 `main.css`에도 동일하게 반영
- 운영 반영 내용
  - `.q-row` gap `0`
  - `.q-num` 폭 `18px`, 좌측 정렬, tabular 숫자 폭 적용
  - `.q-options` 정렬 보조, `.q-opt` 고정 폭 유지
  - `index.html`의 `main.css` 버전을 `202604061103`으로 갱신해 캐시 우회

## 2026-04-06 고급 기능 안내 문구 개편
- `index.html`
  - 일반모드용 `고급 기능 안내` 문구를 기능 가치 중심 설명으로 개편
  - 고급모드용 `고급 기능` 문구를 기능 이득이 더 직접 보이도록 개편
  - `전체 시간 75분 고정`과 `4분 빨리 끝나는 문제 보정` 문구는 고급모드 설명에만 유지
- `staging/site/index.html`
  - 운영과 동일한 문구로 동기화
- 핵심 반영
  - `문항별 상세 통계 TXT 다운로드`는 과목별 정오답, 미응답, 문항별 소요 시간을 복기 자료로 남길 수 있다는 점을 짧게 강조
  - 다른 항목도 단순 기능 설명이 아니라 실전 감각, 복기, 약점 분석, 시간 운영 연습에 어떤 이득이 있는지 보이도록 수정
  - 일반모드 안내와 고급모드 안내를 공용 문구로 합치지 않고 별도로 유지
  - `계산기 고급 동작`, `우측 버튼 열`처럼 실제와 가장 가깝게 구현한 축은 안내 상단으로 재배치
  - 문구는 구현 디테일을 그대로 드러내지 않도록 한 단계 추상화해서 설명

## 2026-04-06 건너뛴 문제 정답 입력 UX 보강
- 확인 결과
  - 로직상 건너뛴 문제도 `정답 입력 모드`에서는 이미 클릭해서 정답을 입력할 수 있었음
  - 불편 제보는 기능 부재보다 UI 안내 부족에 가까웠음
- 반영 내용
  - `index.html`, `staging/site/index.html`에 `omrModeHint` 안내 영역 추가
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`에서 정답 입력 모드 진입 시 `미응답(건너뛴) 문제도 번호를 바로 클릭해 정답을 입력할 수 있습니다.` 문구 노출
  - `main.css`, `staging/site/assets/styles/main.css`에 안내 스타일 추가
  - 캐시 우회를 위해 운영/스테이징 자산 버전을 `202604061155`로 갱신
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 서버 `http://127.0.0.1:8124/index.html` 응답 200 확인
  - 브라우저 MCP가 현재 연결되지 않아 실제 클릭 자동화 검증은 수행하지 못함
