# 2026-04-05 스테이징 팝업 UI 정렬 및 계산기 개선 작업 기록
작성일시: 2026-04-08 15:45:00 KST

## 사용자 요청
- 운영 반영 전, `staging/site`에서 먼저 개선 작업 진행
- 실제 SKCT 우측 화면처럼 하단 여백을 둘 수 있게 하고, 우측 상단 채팅 버튼 자리와 우측 하단 물음표 버튼 자리 추가
- 메모장/그림판/삭제 상단 UI와 계산기 버튼 디자인을 실제 화면에 더 가깝게 조정
- 계산기는 이전 계산 결과 3개 + 현재 계산 1개를 쌓아 보여주고, `=` 후 연산 기호를 이어 쓰는 일반 계산기 흐름 지원
- 메모장 글씨 크기, 그림판 굵기 조절 기능 추가
- 위 기본값은 개발자 페이지에서만 저장 가능하고 Firebase 스테이징 경로에 저장
- 후원 목록을 사용 가이드가 아니라 `커피후원` 모달 최상단으로 옮기기
- 이후 반복 지시를 줄이기 위해, `일반모드 / 고급모드 / 스테이징 / 운영` 같은 표준 용어를 문서로 고정
- OMR 문항 번호 열 정렬과 선택지 버튼 간격을 더 촘촘하게 정리
- 고급 기능 안내 문구를 일반모드용/고급모드용으로 분리 유지한 상태에서, 더 설명력 있고 구매 유도형 문장으로 정리
- 건너뛴 문제도 정답 입력이 가능한지 로직 확인 후, 실제 동작을 더 명확히 보여주는 안내 문구를 OMR에 추가

## 2026-04-06 지시 용어집 추가
- 새 문서 `docs/SKCT_TOOL_지시_용어집.md` 생성
- `normal 모드 -> 일반모드`, `고급버전 -> 고급모드`, `테스트사이트 -> 스테이징`, `실서버 -> 운영` 기준을 고정
- 단순 용어 치환만이 아니라 `스테이징 반영`, `운영 반영`, `이식`, `기본값 저장`처럼 작업 지시 문장도 같이 표준화

## 2026-04-06 OMR 정렬 보정
- `staging/site/assets/styles/main.css`
  - 문항 번호 영역(`.q-num`) 폭을 고정해서 1자리/2자리 번호가 같은 시작선에 맞도록 조정
  - 선택지 행(`.q-options`)의 간격을 더 좁게 유지하도록 구조 정리
  - 추가 보정으로 번호 텍스트 정렬을 오른쪽에서 왼쪽으로 바꿔 `1.` 앞 여백처럼 보이는 현상을 제거
  - 추가 보정으로 행 gap을 0으로 줄이고 번호 칸 폭도 `18px`까지 타이트하게 줄여 `nn.` 뒤 공백처럼 보이는 영역을 최소화
- `staging/site/assets/scripts/app.bundle.js`
  - OMR 렌더링 시 인라인 `gap:4px`를 제거하고 CSS 공통값만 따르도록 수정
- `staging/site/index.html`
  - CSS/JS 캐시가 남지 않도록 자산 버전을 `202604061050`으로 갱신
- 기대 효과
  - `1.`과 `10.` 이후 번호가 서로 밀리지 않음
  - 1~5 선택지 간 간격이 줄어들어 좁은 OMR 폭에서도 공간 활용이 좋아짐
  - `nn.` 앞뒤 공백처럼 보이는 영역이 최소화되어 번호와 선택지가 더 촘촘하게 붙어 보임

## 2026-04-06 운영 이식
- staging에서 승인된 OMR 번호 정렬/간격 보정을 운영 `main.css`에도 동일하게 반영
- 운영 반영 내용
  - `.q-row` gap `0`
  - `.q-num` 폭 `18px`, 좌측 정렬, tabular 숫자 폭 적용
  - `.q-options` 정렬 보조, `.q-opt` 고정 폭 유지
  - `index.html`의 `main.css` 버전을 `202604061103`으로 갱신해 캐시 우회

## 2026-04-06 고급 기능 안내 문구 개편
- `index.html`
  - 일반모드용 `고급 기능 안내` 문구를 기능 가치 중심 설명으로 개편
  - 고급모드용 `고급 기능` 문구를 기능 이득이 더 직접 보이도록 개편
  - `전체 시간 75분 고정`과 `4분 빨리 끝나는 문제 보정` 문구는 고급모드 설명에만 유지
- `staging/site/index.html`
  - 운영과 동일한 문구로 동기화
- 핵심 반영
  - `문항별 상세 통계 TXT 다운로드`는 과목별 정오답, 미응답, 문항별 소요 시간을 복기 자료로 남길 수 있다는 점을 짧게 강조
  - 다른 항목도 단순 기능 설명이 아니라 실전 감각, 복기, 약점 분석, 시간 운영 연습에 어떤 이득이 있는지 보이도록 수정
  - 일반모드 안내와 고급모드 안내를 공용 문구로 합치지 않고 별도로 유지
  - `계산기 고급 동작`, `우측 버튼 열`처럼 실제와 가장 가깝게 구현한 축은 안내 상단으로 재배치
  - 문구는 구현 디테일을 그대로 드러내지 않도록 한 단계 추상화해서 설명

## 2026-04-06 건너뛴 문제 정답 입력 UX 보강
- 확인 결과
  - 로직상 건너뛴 문제도 `정답 입력 모드`에서는 이미 클릭해서 정답을 입력할 수 있었음
  - 불편 제보는 기능 부재보다 UI 안내 부족에 가까웠음
- 반영 내용
  - `index.html`, `staging/site/index.html`에 `omrModeHint` 안내 영역 추가
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`에서 정답 입력 모드 진입 시 `미응답(건너뛴) 문제도 번호를 바로 클릭해 정답을 입력할 수 있습니다.` 문구 노출
  - `main.css`, `staging/site/assets/styles/main.css`에 안내 스타일 추가
  - 캐시 우회를 위해 운영/스테이징 자산 버전을 `202604061155`로 갱신
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 서버 `http://127.0.0.1:8124/index.html` 응답 200 확인
  - 브라우저 MCP가 현재 연결되지 않아 실제 클릭 자동화 검증은 수행하지 못함

## 2026-04-06 추가 반영
- `staging/site/index.html`
  - `id="sponsorTicker"` 컨테이너를 `helpModal`에서 제거
  - 같은 컨테이너를 `donateModal` 본문 최상단으로 이동
  - 기존 후원 목록 렌더링 로직과 속도 설정 로직은 그대로 유지
- 영향 범위
  - normal / 고급 버전 모두 동일한 후원 모달 템플릿을 공유하므로 staging 파일 한 곳 수정으로 두 모드에 함께 반영

## 2026-04-08 운영 반영: 고급 안내 기반 수동 구독 신청/조회 플로우
- 사용자 승인 후 운영 반영 대상으로 로컬에 쌓여 있던 수동 구독 신청 시스템과 저부하 접속자 집계 개선을 함께 점검
- `index.html`
  - 일반모드 `고급 안내` 모달을 `1. 로그인 -> 2. 고급 기능 -> 3. 구독 등록 방법` 순서로 재구성
  - 기간별 가격 카드, 투네이션 후원 버튼, 비밀글형 신청 순서를 한 화면에서 이해할 수 있게 정리
  - 구독 신청 양식을 `구독권 선택 / 투네이션 이름 / 이용 시작일 / ID / 닉네임 / 이메일 / 조회 비밀번호` 중심의 칸 채우기 방식으로 변경
  - 신청 저장 후 `신청번호 + 조회 비밀번호`로 다시 조회하는 흐름을 유지하고, `내 비밀글 조회` 문구로 역할을 명확히 함
  - `subscription-crypto.js`, `main.js` 캐시 버전을 `202604081336`으로 갱신
- `main.js`
  - 수동 신청 기본값 설정 로직에 `이용 시작일` 자동 기본값(KST 오늘 날짜) 추가
  - 신청 payload에 `requestedStartDate`를 저장하고, 조회 결과에도 이용 시작일/신청 메모가 보이도록 보강
  - 제출 검증 문구를 실제 신청 항목 기준으로 다시 정리
  - 비밀글 저장 성공 메시지로 문구를 교체
  - 동시에 기존 로컬 개선분인 실시간 접속자 구조(`fetch` 반복 제거, `onValue` 구독, `serverTimestamp` heartbeat, 일별 peak snapshot)도 운영 반영 후보에 포함
- `admin.html`
  - 수동 구독 신청 관리 섹션을 운영용 구조로 포함
  - 운영자 안내 문구에 실제 신청 양식 항목을 명시
  - 복호화 후 보이는 신청 본문에도 `구독권 / 이용 시작일 / 로그인 ID / 닉네임 / 이메일`이 같은 형식으로 드러나도록 정리
  - 승인 시 생성되는 고급 구독 note에도 `이용 시작일`을 남기도록 보강
- `subscription-crypto.js`
  - 사용자 비밀번호와 운영자 공개키로 각각 접근 경로를 분리하는 브라우저 측 암호화 모듈을 운영 자산으로 포함
- 로컬 검증
  - `node --check main.js`
  - `node --check subscription-crypto.js`
  - 로컬 정적 서버에서 `index.html`, `admin.html` 응답 `200`
  - HTML 응답 문자열 기준으로 `1. 로그인`, `2. 고급 기능`, `3. 구독 등록 방법`, `manualSubscriptionStartDateInput`, `비밀 구독 게시판 작성`, `subscription-crypto.js?v=202604081336` 반영 확인
- 주의
  - 실제 신청 저장/승인까지의 end-to-end 검증은 운영 Firebase 쓰기 경로를 건드리므로 이번 로컬 검증에서는 실행하지 않음
  - 따라서 운영 반영 후에는 실신청 없이 화면/문구/조회 동선만 먼저 점검하는 것이 안전함

## 2026-04-08 Codex Windows 환경 잔재 정리
- 원인 확인
  - `C:\Users\kshcg\.codex\bin\python`, `python3`, `python3.12`, `pip`, `pip3`, `pip3.12`가 Windows에서 직접 실행하기 어려운 링크/유닉스 shebang 형태로 PATH 상단에 남아 있었음
  - `python` 또는 `pip` 명령 시 실제 설치 경로보다 `.codex\bin` 쪽이 먼저 잡혀 앱 선택 창이 뜰 수 있는 상태였음
- 조치
  - 위 파일들을 `.disabled_YYYYMMDD_HHMMSS` 형식으로 이름 변경해 비활성화
  - 추가로 `.cmd` 대체제가 있는 bare bash 스크립트 `codex`, `codex-notify`, `codex-playwright-mcp`, `codex-chrome-devtools`, `codex-mcp-rsync`, `codexmulti`도 같은 방식으로 비활성화
- 검증
  - `where python` -> 실제 `Python313\\python.exe`
  - `where pip`, `where pip3` -> 실제 `Python313\\Scripts\\pip.exe`, `pip3.exe`
  - `Get-Command codex*` -> 모두 `.cmd` 래퍼 사용
  - `codex --version` 정상
  - `codex-playwright-mcp --help` 정상
  - `python --version`, `pip --version`, `python -m pip --version` 정상

## 2026-04-07 고급모드 타이머 폭/버튼 대비 보정
- 사용자 요청
  - 고급모드 타이머가 여백을 너무 많이 써서 OMR을 펼치면 우측 조작 버튼이 화면 밖으로 밀림
  - 타이머 버튼 색 대비가 낮아 잘 안 보임
- 반영 파일
  - `main.css`
  - `staging/site/assets/styles/main.css`
- 반영 내용
  - 고급모드 타이머 셸의 좌우 패딩, gap, 최소 폭을 다시 줄여 상단 공간을 더 효율적으로 사용하도록 조정
  - 전체 시간/과목 시간 블록의 최소 폭과 글자 크기를 소폭 줄여 한 줄 유지력을 높임
  - `▶ / 다음 / 과↺ / 전↺` 버튼 그리드를 `36px` 기반으로 재구성하고, 좁은 폭에서는 `32px`까지 더 줄어들게 조정
  - 재생/다음/과목 초기화/전체 초기화 버튼 색을 더 진하고 분명한 톤으로 바꾸고, 비활성 상태도 글자 대비를 유지하도록 수정
  - 고급모드 상단 바와 팝업 버튼의 기본 패딩도 함께 줄여 타이머가 먹는 폭을 더 줄임
- 기대 효과
  - OMR 확장 시 상단 타이머 때문에 우측 조작 버튼이 밀리는 현상이 줄어듦
  - 타이머 버튼 구분과 비활성 상태 가독성이 더 좋아짐
- 검증
  - `node --check main.js`
  - 로컬 서버 `http://127.0.0.1:8125/index.html?advanced=1` 응답 200 및 화면 로딩 확인

## 2026-04-07 고급모드 타이머 좌측 정렬/빈공간 축소
- 사용자 요청
  - 타이머 블록을 좌측 정렬하고, `전체 남은 시간` 옆의 검은 빈공간을 최소화
  - 흰색 여백처럼 보이는 정렬 어긋남도 함께 제거
- 반영 파일
  - `main.css`
  - `staging/site/assets/styles/main.css`
  - `index.html`
  - `staging/site/index.html`
- 반영 내용
  - 고급모드 타이머 셸의 `margin-left: auto`를 제거하고 `justify-content: flex-start`로 변경
  - 타이머 셸이 남는 폭을 억지로 차지하지 않도록 `flex: 0 1 auto`, `width: auto`로 조정
  - `timer-total-block`, `timer-phase-block`을 좌측 정렬로 맞춰 텍스트 시작선이 한쪽으로 정리되도록 수정
  - 운영/스테이징 CSS 버전을 `202604071255`로 갱신
- 기대 효과
  - 타이머 블록이 상단에서 좌측으로 더 단단하게 붙고, 검은 빈칸이 줄어듦
  - 숫자/라벨 시작선이 정리되어 흰 여백처럼 보이던 어색함이 완화됨
- 검증
  - `node --check main.js`
  - 로컬 서버 `http://127.0.0.1:8125/index.html?advanced=1` 응답 200
  - Playwright로 고급모드 상단 타이머 블록 로딩 확인

## 2026-04-06 스테이징 반영: 고급모드 정오표 일괄입력
- 사용자 요청
  - `docs/TODO/TODO1.md` 기준으로, 정답 입력 모드에서 링커리어 스타일 정오표 표를 한 번에 붙여넣어 정답을 일괄 반영하는 기능 추가
  - 여러 형식을 최대한 자동 인식하고, 자동 인식이 애매하면 사용자가 문항 번호 열과 정답 열을 직접 고를 수 있게 구성
  - 일반모드가 아니라 고급모드에서만 제공
- 반영 파일
  - `staging/site/index.html`
    - 고급모드 + 정답 입력 상태에서만 보이는 `📥 정오표 일괄입력` 버튼 추가
    - 붙여넣기/열 선택/미리보기/적용 흐름을 담는 전용 모달 추가
  - `staging/site/assets/styles/main.css`
    - 일괄입력 모달, 요약, 열 선택, 미리보기 테이블 스타일 추가
  - `staging/site/assets/scripts/app.bundle.js`
    - 반복 헤더 블록(`NO. 정답 입력답 정오 정답률`) 자동 무시
    - 탭, 다중 공백, 파이프, 쉼표 구분 형식 파싱
    - 문항 번호 열 / 정답 열 자동 탐지
    - 자동 탐지가 애매할 때 수동 선택 UI 제공
    - 미리보기 후 `omrState.correctAnswers`에 일괄 반영하는 적용 로직 추가
- 동작 방식
  - 정오표 텍스트를 붙여넣고 `미리보기 생성`을 누르면 읽어낸 문항 수와 후보 열을 보여줌
  - 자동 선택이 맞으면 그대로 적용하고, 아니면 드롭다운에서 문항 번호 열과 정답 열을 바꾼 뒤 다시 미리보기를 확인 가능
  - 적용 후에는 현재 시험 OMR 정답 데이터에 즉시 반영되어 오답풀이/채점 기준으로 그대로 사용됨
- 로컬 검증
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 서버 `http://127.0.0.1:8131/staging/site/index.html?stage=1&preview=1&advanced=1`에서 고급모드 진입 후 브라우저 검증
  - 샘플 데이터
    - `NO. 정답 입력답 정오 정답률` 헤더가 두 번 반복되는 표 텍스트 사용
    - `1, 2, 3, 11, 12, 13번` 문항에 대해 정답 열 자동 인식 확인
  - 결과
    - 요약 문구 `데이터 줄 6개 중 6문항을 정답으로 읽었습니다.`
    - 자동 선택 `NO. / 정답`
    - 적용 후 `6문항의 정답을 반영했습니다.`
    - 실제 OMR DOM에서 각 문항의 정답 선택지가 `.selected.correct`로 반영된 것 확인
- 배포 범위
  - 이번 턴은 스테이징만 반영
  - 운영은 사용자 확인 후 승인 시에만 이식
  - staging 확인 후 같은 변경을 운영 `index.html`에도 이식

## 2026-04-06 운영 이식: 고급모드 정오표 일괄입력
- 사용자 승인 후 운영 `index.html`, `main.js`, `main.css`에 스테이징에서 검증한 동일 기능 이식
- 운영 반영 내용
  - 고급모드 정답 입력 상태에서만 `📥 정오표 일괄입력` 버튼 노출
  - 정오표 붙여넣기 모달, 자동 열 인식, 수동 열 선택, 미리보기, 일괄 반영 흐름 운영 적용
  - `main.css`, `main.js` 자산 버전을 `v202604061825`로 갱신해 캐시 회피 강화
  - 일반모드/고급모드의 고급 안내 문구에도 `정오표 일괄입력`을 3번째 항목으로 반영하고, 문항 번호가 없어도 정답 열 중심 형식은 불러올 수 있다는 설명 추가
- 운영 검증
  - `node --check main.js`
  - 로컬 코드 기준 `bulkCorrectImport*` DOM/파서/적용 로직 존재 확인
  - 원격 반영은 GitHub Pages 배포 지연을 감안해 push 후 별도 확인 예정
- 반영 커밋
  - 운영 코드: `a7ec4f7`

## 이번 턴에서 수정한 대상
- `staging/site/index.html`
- `staging/site/assets/styles/main.css`
- `staging/site/assets/scripts/app.bundle.js`
- `staging/site/admin.html`

## 구현 내용
- 스테이징 사용자 화면
  - 상단 툴 영역을 `메모장 / 그림판 / 삭제` 구조로 정리
  - 메모장 글씨 크기 슬라이더, 그림판 굵기 슬라이더 추가
  - 계산기 표시부를 입력창 방식에서 히스토리 스택 방식으로 변경
  - 계산 결과 최대 3개 이력 + 현재 계산 1개를 우측 하단 정렬로 표시
  - `C`, `←`, `÷`, `×`, `-`, `+`, `00`, `=` 포함 계산기 버튼 배열로 변경
  - 우측 상단 채팅 버튼 자리, 우측 하단 물음표 버튼 자리 추가
  - 두 버튼 클릭 시 `skct와 동일한 위치에 존재하는 기능 없는 버튼입니다` 알림 표시
- 스테이징 팝업 편집기
  - 팝업 편집 모드에서 상단/중간 가로 분리선 드래그 가능
  - 하단 여백 비율 슬라이더 추가
  - 팝업 편집기 안에서 조절된 메모 글씨 크기, 그림판 굵기까지 함께 저장 대상에 포함
- 스테이징 관리자 페이지
  - `팝업 편집기 열기` 버튼 추가
  - 현재 스테이징 팝업 기본값 요약 박스 추가
  - 팝업 편집기에서 전달한 값을 받아 아래 경로에 저장하도록 연결
    - `staging_hidden_v1/config/popupLayout`
    - `staging_hidden_v1/config/layoutRatios`
    - `staging_hidden_v1/config/toolUiConfig`
  - 스테이징 링크를 `stage=1` 게이트와 관리자 접근 토큰(`stg_skct_admin_gate_until`)을 사용하도록 정리

## 추가된 기본값 구조
- `popupLayout`
  - `window.widthRatio`
  - `window.heightRatio`
  - `window.leftRatio`
  - `window.topRatio`
  - `omrWidthRatio`
- `layoutRatios`
  - `timer`
  - `utils`
  - `calc`
- `toolUiConfig`
  - `bottomPaddingRatio`
  - `noteFontSize`
  - `canvasLineWidth`

## 검증 결과
- `node --check staging/site/assets/scripts/app.bundle.js` 통과
- 로컬 서버 `http://127.0.0.1:8000` 기준 브라우저 검증
  - `staging/site/index.html?stage=1&preview=1` 접근 확인
  - `stg_skct_popup_mode` 팝업 편집 모드에서 편집 패널, 리사이저, 숨김 처리 정상 확인
  - 메모장 글씨 크기 조절 시 `18px` 반영 확인
  - 그림판 탭 전환 후 굵기 값 `7` 반영 확인
  - 계산기 `2 + 3 =`, 이어서 `+ 4 =` 수행 시 히스토리 `2 + 3 = 5`, `5 + 4 = 9`, 현재값 `9` 누적 확인
  - 물음표 버튼 클릭 시 실제 알림창 발생 후 수동 수락 확인
  - 스테이징 사용자 화면 / 관리자 화면 모두 콘솔 에러·경고 없음 확인

## 운영 반영 상태
- 이번 턴 변경은 `staging/site`에만 적용했습니다.
- 운영본(`index.html`, `main.js`, `main.css`, `admin.html`)에는 아직 반영하지 않았습니다.

## 스테이징 배포 계획
- 운영 GitHub Pages는 `main` 루트가 실사용자 화면이므로, 이번 배포에서는 `staging/site/*`와 관련 작업 기록만 선택적으로 커밋합니다.
- 서버 관리자 페이지에는 이미 `🧪 테스트 사이트` 진입 버튼이 배포되어 있으므로, 스테이징 경로 파일만 갱신해도 개발자 페이지에서 새 테스트 화면을 열 수 있습니다.
- 운영 사용자 화면 루트 파일은 이번 배포 범위에서 제외합니다.

## 스테이징 배포 결과
- 선택 커밋: `6db4fe7` (`Deploy staging popup editor UI improvements`)
- 원격 반영: `origin/main` push 완료
- 원격 확인
  - `https://agenticlab-sh.github.io/skct_tool/staging/site/index.html`에서 `mockQuestionBtn`, `calcHistory`, `noteFontSizeRange` 문자열 확인

## 추가 수정: 하단 여백 절대 비율화 및 최소 높이 제한 제거
- 사용자 요청
  - 테스트 페이지에서 하단 여백을 더 크게 줄 수 있게 하고, 이 값은 남은 레이아웃과 섞이지 않는 절대 비율로 계산
  - 기본값을 하단 여백 `20%`, 우측 버튼 열 `12.5%`로 조정
  - 타이머, 메모장, 계산기 영역 최소 높이 제한 제거
  - 테스트 조정 시 실제 화면과 가깝게 보기 위해 상단 테스트 배너 제거
- 수정 내용
  - `staging/site/assets/scripts/app.bundle.js`
    - `DEFAULT_TOOL_UI_CONFIG` 기본값을 `bottomPaddingRatio: 0.20`, `sideButtonColumnRatio: 0.125`로 변경
    - 하단 여백 계산 기준을 `mainContent` 높이 대신 `screen.availHeight` 기반으로 변경
    - 하단 여백 저장 상한을 `0.9`까지 확장
    - 팝업 편집 드래그 리사이즈의 `MIN_TIMER_HEIGHT`, `MIN_UTILITY_HEIGHT`, `MIN_CALC_HEIGHT`를 `0`으로 완화
  - `staging/site/index.html`
    - 팝업 편집기 슬라이더 기본값을 하단 `20%`, 우측 버튼 열 `12.5%`로 변경
    - 하단 여백 슬라이더 최대값을 `0.9`로 확장
    - 상단 읽기 전용 안내 배너 제거
  - `staging/site/assets/styles/main.css`
    - `.top-bar`, `.utility-section`, `.calculator-section`, `.calc-display-shell`, `.calc-history`, `.calc-keys`의 최소 높이 제한을 제거하거나 완화
    - 계산기 버튼 그리드의 행 최소값을 `minmax(0, 1fr)`로 변경
  - `admin.html`
    - 스테이징 저장값을 다시 읽을 때 하단 여백과 우측 버튼 열 기본값/상한이 잘리지 않도록 보정 범위 갱신
- 검증 결과
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - `git diff` 기준으로 staging 조정 관련 파일만 수정됨 확인
- 비고
  - 브라우저 주소창과 메뉴 영역은 웹 페이지 코드만으로 완전히 제거할 수 없습니다. 그래서 대신 팝업창과 레이아웃 최소 높이 제한을 느슨하게 풀어 실제 시험 화면 비율에 더 가깝게 맞추는 방향으로 대응했습니다.

## 추가 수정: 우측 버튼 열 하한 해제 및 현재 조정값을 새 기본값으로 반영
- 사용자 관찰
  - 계산기 비율을 줄이면 버튼 글자와 계산 표시 글자가 버튼/표시 영역을 벗어남
  - 우측 버튼 열 비율을 더 줄여도 실제 폭이 거의 줄지 않음
  - 현재 맞춰 둔 값
    - 하단 여백 `11.0%`
    - 우측 버튼 열 `9.0%`
    - 창 크기 `27.3% x 98.0%`
    - 창 위치 `왼쪽 72.7% / 위 0.0%`
    - OMR 폭 `34.0%`
    - 세로 비율 `타이머 8.4 / 메모 47.5 / 계산기 44.1`
- 원인
  - 우측 버튼 열은 비율 슬라이더 하한뿐 아니라 실제 버튼 픽셀 크기가 `44px` 최소값으로 막혀 있어서, 작은 값으로 내려도 육안상 거의 안 줄어들었음
  - 계산기 내부는 영역 비율만 줄고 폰트 크기, 버튼 모서리, 간격은 고정값이어서 축소 시 글자가 쉽게 넘쳤음
- 수정 내용
  - `staging/site/assets/scripts/app.bundle.js`
    - 기본 팝업 레이아웃을 현재 값으로 변경
      - 창 `0.273 x 0.98`
      - 위치 `0.727 / 0`
      - OMR 폭 `0.34`
    - 기본 세로 비율을 `8.4 / 47.5 / 44.1`로 변경
    - 기본 도구 비율을 `하단 0.11`, `우측 버튼 열 0.09`로 변경
    - 우측 버튼 열 하한을 `0.03`까지 낮춤
    - 실제 버튼 크기 하한을 `44px -> 22px`로 낮춤
  - `staging/site/assets/styles/main.css`
    - 계산기 히스토리 글자, 현재 줄 글자, 버튼 글자 크기를 `clamp()` 기반 반응형으로 변경
    - 계산기 버튼 간격, 하단 패딩, 테두리 반경도 축소에 따라 같이 줄도록 변경
    - 우측 버튼을 예약 열 안에서 가운데 정렬되도록 바꿔 열 폭 변화가 눈에 보이게 정리
    - 채팅/물음표 버튼 아이콘도 버튼 크기를 따라 같이 축소되도록 변경
  - `staging/site/index.html`
    - 팝업 편집 슬라이더 기본값을 `하단 11.0%`, `우측 버튼 열 9.0%`로 변경
    - 우측 버튼 열 슬라이더 최소값을 `0.03`으로 낮춤
  - `admin.html`
    - 관리자 페이지의 기본 팝업 값, 세로 비율, 도구 기본값도 같은 수치로 맞춤
- 검증 결과
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - 코드 기준으로 우측 버튼 열 하한 `0.03`, 버튼 최소 크기 `22px`, 새 기본값 `0.11 / 0.09 / 0.273 / 0.727 / 8.4 / 47.5 / 44.1` 반영 확인
  - staging Firebase 실제값도 아래처럼 직접 확인
    - `staging_hidden_v1/config/toolUiConfig.json` → `bottomPaddingRatio: 0.11`, `sideButtonColumnRatio: 0.09`, `noteFontSize: 14`, `canvasLineWidth: 4`
    - `staging_hidden_v1/config/popupLayout.json` → `window 0.273 / 0.98 / 0.727 / 0`, `omrWidthRatio: 0.34`
    - `staging_hidden_v1/config/layoutRatios.json` → `timer: 8.4`, `utils: 47.5`, `calc: 44.1`

## 추가 수정: 메모/그림 기본값 재조정 및 도구 헤더 높이 축소
- 사용자 요청
  - 그림판 선 굵기 기본값 `2`
  - 메모장 글씨 크기 기본값 `12`
  - 메모장/그림판/삭제가 있는 상단 도구 헤더 높이를 현재 대비 약 `70%` 수준으로 축소
  - 아래 값을 새 기본값으로 반영
    - 하단 여백 `11.0%`
    - 우측 버튼 열 `9.0%`
    - 창 크기 `26.9% x 98.0%`
    - 창 위치 `왼쪽 73.1% / 위 0.0%`
    - OMR 폭 `34.0%`
    - 세로 비율 `타이머 8.6 / 메모 45.0 / 계산기 46.4`
    - 도구 기본값 `메모 12px / 그림판 2px`
- 수정 내용
  - `staging/site/assets/scripts/app.bundle.js`
    - 기본 팝업 레이아웃을 `0.269 / 0.98 / 0.731 / 0`으로 변경
    - 기본 세로 비율을 `8.6 / 45.0 / 46.4`로 변경
    - 기본 도구값을 `bottomPaddingRatio: 0.11`, `sideButtonColumnRatio: 0.09`, `noteFontSize: 12`, `canvasLineWidth: 2`로 변경
  - `staging/site/index.html`
    - 설정 슬라이더 초기값을 메모 `12`, 그림판 `2`로 변경
  - `staging/site/assets/styles/main.css`
    - 도구 헤더 패딩 `9/12/8 -> 6/10/5`
    - 탭 간격 `18px -> 12px`
    - 탭 글자 `14px -> 12px`
    - 삭제 버튼 패딩 `8x16 -> 5x11`, 글자 `13px -> 12px`
  - `admin.html`
    - 관리자 페이지의 기본 팝업/비율/도구 기본값도 같은 수치로 갱신
- staging Firebase 실제값 재확인
  - `staging_hidden_v1/config/toolUiConfig.json` → `bottomPaddingRatio: 0.11`, `sideButtonColumnRatio: 0.09`, `noteFontSize: 12`, `canvasLineWidth: 2`
  - `staging_hidden_v1/config/popupLayout.json` → `window 0.269 / 0.98 / 0.731 / 0`, `omrWidthRatio: 0.34`
  - `staging_hidden_v1/config/layoutRatios.json` → `timer: 8.6`, `utils: 45.0`, `calc: 46.4`

## 추가 수정: 계산기 표시부 4줄 고정 체감 및 버튼 영역 확대
- 사용자 요청
  - 계산기 내역 표시 칸은 4줄만 보이는 크기로 조정
  - 현재 라인은 이전 3줄보다 조금 더 큰 글씨로 표시
  - 비워진 공간만큼 계산기 버튼 높이를 늘림
- 수정 내용
  - `staging/site/assets/styles/main.css`
    - 표시부를 `flex: 0 0 auto` + 고정 높이 `clamp(84px, 17vh, 104px)`로 제한
    - 표시부 패딩과 줄 간격을 줄여 3개 이력 + 현재값 1개가 안정적으로 들어오게 조정
    - 현재 줄 폰트를 `clamp(15px, 5vw, 26px)`로 키우고, 이력 줄은 더 작게 축소
    - 버튼 그리드를 `flex: 1 1 auto`로 바꿔 남는 높이를 버튼이 더 가져가도록 조정
- 검증 결과
  - CSS diff 기준 표시부 높이 고정, 현재 줄 확대, 버튼 영역 flex 확대 반영 확인

## 운영 반영 완료
- 사용자 승인 후 운영 반영 수행
- 운영 반영 코드 커밋: `862ee50`
- 운영 반영 대상
  - `index.html`
  - `main.css`
  - `main.js`
- 운영 반영 내용
  - 승인된 팝업 기본값, 세로 비율, 하단 여백, 우측 버튼 열, 메모 12px, 그림판 2px 이식
  - 우측 채팅/물음표 자리 버튼 이식
  - 메모/그림/삭제 도구 헤더 밀도 축소 이식
  - 계산기 표시부 4줄 체감, 현재 줄 확대, 버튼 높이 재배분 이식
  - 운영 팝업 편집 패널에 하단 여백 / 우측 버튼 열 슬라이더 이식
- 운영 Firebase 실제값 확인
  - `config/popupLayout.json` → `{"omrWidthRatio":0.34,"window":{"heightRatio":0.98,"leftRatio":0.731,"topRatio":0,"widthRatio":0.269}}`
  - `config/layoutRatios.json` → `{"calc":46.4,"timer":8.6,"utils":45.0}`
  - `config/toolUiConfig.json` → `{"bottomPaddingRatio":0.11,"canvasLineWidth":2,"noteFontSize":12,"sideButtonColumnRatio":0.09}`
  - `https://agenticlab-sh.github.io/skct_tool/staging/site/admin.html`에서 `popupLayoutEditorBtn`, `popupLayoutSummary`, `toolUiConfig` 문자열 확인
  - 운영 관리자 페이지 `https://agenticlab-sh.github.io/skct_tool/admin.html`에는 기존 `🧪 테스트 사이트` 버튼이 유지됨 확인
- 영향 범위
  - 실사용 루트 페이지(`index.html`, `main.js`, `main.css`)는 이번 배포에 포함하지 않음
  - 숨은 스테이징 경로(`staging/site/*`)만 갱신

## 참고 사진 기준 최종 시각 점검
- 점검 대상
  - 사용자 첨부 계산기 참고 사진
  - `docs/참고사진/image copy.png`
  - 원격 스테이징 팝업 `https://agenticlab-sh.github.io/skct_tool/staging/site/index.html?stage=1&preview=1`
- 확인 결과
  - 일치한 부분
    - 상단 탭이 `메모장 / 그림판 / 삭제` 구조로 보임
    - 계산기 표시부가 우측 하단 정렬로 표시됨
    - `=` 버튼이 진한 색으로 강조됨
    - 우측 상단 채팅 버튼, 우측 하단 물음표 버튼 자리 자체는 존재함
  - 아직 다른 부분
    - 현재 팝업은 “우측 도구 패널만” 뜨는 형태가 아니라, 왼쪽 GUIDE/OMR 사이드바와 상단 배너가 같이 보이는 축소 화면 형태임
    - 계산기 첫 줄 우측 버튼이 참고 사진의 `√`가 아니라 현재 `×`로 배치되어 있음
    - 참고 사진의 `×`는 둘째 줄 맨 오른쪽인데, 현재 배열은 한 줄씩 위로 당겨진 형태임
    - 표시부 안의 이전 계산 3줄은 참고 사진처럼 희미하게 누적된 모습까지는 아직 완전히 맞지 않고, 초기 상태에서는 `0`만 강하게 보임
    - 우측 하단 물음표 버튼은 현재 뷰포트 기준 일부만 걸쳐 보이며, 참고 사진처럼 버튼 전체가 안정적으로 노출되지는 않음
    - 전체 여백/간격도 참고 사진보다 조금 넓어 실제 시험 화면보다 더 여유 있게 보임
- 결론
  - 기능 방향은 대부분 반영되었지만, 참고 사진과 “최종 동일” 수준은 아직 아닙니다.
  - 특히 팝업 전체 구조와 계산기 버튼 배열은 한 번 더 맞춤 보정이 필요합니다.

## 3단계 반영 프로세스 추가
- 사용자 요청
  - 테스트 사이트에서 조정한 값은 저장 버튼을 눌러야 테스트 기본값으로 반영
  - 개발자 페이지에서 `테스트 저장 -> 테스트 화면 확인 -> 실제 서버 반영` 흐름을 단계별 버튼으로 분리
- 이번 추가 수정
  - `staging/site/index.html`
    - 테스트 사용자 페이지가 `config`가 아니라 `staging_hidden_v1/config`를 읽도록 수정
    - 팝업 편집기 저장 버튼 문구를 `테스트 기본값 저장`으로 변경
  - `admin.html`
    - 팝업 기본값 섹션을 3단계 프로세스 UI로 개편
    - `1. 테스트 기본값 편집`
    - `2. 테스트 화면 열기`
    - `3. 실제 서버 반영`
    - 테스트 기본값 요약 / 실제 서버 기본값 요약을 분리 표시
    - 테스트 팝업 편집기 저장 메시지를 받아 `staging_hidden_v1/config/*`에만 저장
    - 마지막 단계 버튼을 눌렀을 때만 `config/popupLayout`, `config/layoutRatios`, `config/toolUiConfig`로 복사
  - `index.html`, `main.js`, `main.css`
  - 운영 사용자 페이지가 `config/toolUiConfig`를 읽어 하단 여백, 메모 글씨 크기, 그림판 굵기 기본값을 적용하도록 연결
  - 기본값이 없으면 기존 동작과 거의 같게 유지되도록 안전 기본값 사용

## 2026-04-06 고급 기능 팝업 구조 수정
- 사용자 정정
  - 숨김 진입 후 열리는 창은 별도 통계 도구창이 아니라, 실제로 사용하는 팝업 웹의 고급 버전이어야 함
  - TXT 다운로드와 비밀번호 목록 관리도 그 고급 버전 사용자 화면 안에 포함되어야 함
- 기존 문제
  - `advanced-tools.html`이 인증 후에도 독립 도구창으로 남아 있어 사용자가 기대한 실제 시험 팝업 흐름과 달랐음
- 수정 방향
  - `advanced-tools.html`
    - 인증 전용 게이트로 축소
    - 비밀번호 성공 시 같은 창을 실제 팝업 이름(`skct_popup_mode`, 스테이징은 `stg_skct_popup_mode`)으로 전환
    - 이어서 `index.html?advanced=1` 또는 `staging/site/index.html?advanced=1`로 이동
  - `index.html`, `staging/site/index.html`
    - `advanced=1` 런타임 플래그 추가
    - 설정 모달 안에 고급 기능 섹션 추가
      - 문항별 상세 통계 TXT 다운로드
      - 허용 비밀번호 목록 조회/저장
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - 고급 인증 성공 상태를 `localStorage`에 30분 저장
    - `activateAdvancedSession()` helper 추가
    - 고급 모드에서만 설정 모달 내 고급 기능 섹션 표시
    - 숨김 트리거가 여는 인증 창도 일반 팝업과 같은 창 크기/위치 로직 재사용
- 로컬 검증
  - `node --check main.js` 통과
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - 로컬 `http://127.0.0.1:8000/index.html?dev=1`
    - 설정 제목 7연타 후 `advanced-tools.html` 인증 창 열림 확인
    - 비밀번호 `0208` 입력 후 인증 창이 `index.html?advanced=1`로 전환됨 확인
    - 전환된 창의 `window.name = skct_popup_mode` 확인
    - 전환된 창에서 설정 모달을 열면 `고급 기능` 섹션, TXT 다운로드 버튼, 비밀번호 목록 기본값 `0208` 노출 확인
- 운영 반영
  - 코드 커밋: `83471ce` (`Turn advanced popup into authenticated advanced app mode`)
  - `main` push 완료

## 2026-04-06 고급 기능 안내 버튼 추가
- 사용자 요청
  - 고급 기능 페이지 안에 `고급 기능 버튼`을 만들고, 누르면 현재 포함된 고급 기능이 무엇인지 설명이 나오게 하기
- 수정 내용
  - `index.html`, `staging/site/index.html`
    - 고급 기능 섹션 맨 위에 `고급 기능 안내` 버튼 추가
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - 버튼 클릭 시 아래 내용을 설명하는 안내창 표시
      - 현재 창이 숨김 인증을 통과한 고급 버전이라는 점
      - 문항별 상세 통계 TXT 다운로드
      - 허용 비밀번호 목록 관리
      - 일반 설정에는 보이지 않고 제목 연타 + 인증으로만 진입 가능하다는 점
- 검증 결과
  - `node --check main.js` 통과
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과

## 2026-04-06 고급 기능 전용 버튼/모달 분리 및 비밀번호 개발자 전용화
- 사용자 정정
  - 고급 기능은 실제 고급 이용자가 쓰는 화면답게 별도 버튼 구획에서 열려야 함
  - 고급 기능 안내에는 실제로 적용된 고급 기능 항목들이 포함되어야 함
  - 고급 기능 비밀번호 목록은 사용자 화면에 보이면 안 되고 개발자 페이지에서만 관리되어야 함
- 수정 내용
  - `index.html`, `staging/site/index.html`
    - 설정 모달에서 고급 기능 블록 제거
    - 좌측 사이드바에 `🔒 고급 기능` 전용 버튼과 구분선 추가
    - 고급 모드일 때만 보이는 `advanced-only` 구조 추가
    - 별도 `advancedFeatureModal` 추가
      - 문항별 상세 통계 TXT 다운로드
      - 쉬는 시간 건너뛰기
      - 이전 문항 재선택 허용과 시간 고정 규칙
      - 문항 건너뛰기 시간 처리 보정
      - 계산기 `Ans` 누적 / 우선순위 / 이어 계산
      - 전체 시간 75분 고정
  - `main.css`, `staging/site/assets/styles/main.css`
    - `advanced-only`, `advanced-toggle` 스타일 추가
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - 고급 기능 버튼 클릭 시 전용 모달 열기
    - 고급 인증 비밀번호를 로컬 저장소가 아니라 원격 `advancedFeatureConfig.passwords`에서 읽도록 변경
    - 사용자 화면에서 비밀번호 저장 UI/로직 제거
  - `admin.html`, `staging/site/admin.html`
    - `고급 기능 인증 비밀번호` textarea 추가
    - 각각 `config/advancedFeatureConfig`, `staging_hidden_v1/config/advancedFeatureConfig` 저장/로드 연결
- 로컬 검증
  - `node --check main.js` 통과
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - 고급 인증 후 `고급 기능` 사이드바 버튼 표시 확인
  - 설정 모달에서 비밀번호 목록 UI 제거 확인
  - 고급 기능 모달 설명에 `쉬는 시간 건너뛰기`, `이전 문항 재선택 허용`, `계산기 고급 동작` 포함 확인
- 운영 반영
  - 코드 커밋: `37045eb` (`Separate advanced user modal from password management`)
- 로컬 검증
  - `node --check main.js` 통과
  - 로컬 `admin.html`에서 3단계 버튼 DOM 존재 확인
  - 로컬 `staging/site/index.html?stage=1&preview=1`에서 실제 읽기 경로가 `staging_hidden_v1/config`로 바뀐 것 확인
  - 로컬 `staging/site/index.html` 팝업 편집기 저장 버튼 문구가 `테스트 기본값 저장`으로 표시되는 것 확인

## 일반 페이지 팝업 편집 패널 노출 회귀 수정
- 증상
  - 일반 운영 페이지(`index.html?dev=1&preview=1`)에서도 팝업 기본값 편집 패널과 분리선이 보임
- 원인
  - 운영 `index.html`의 팝업 편집 패널은 `class="hidden"`으로 들어가 있었지만, 운영 `main.css`에는 범용 `.hidden { display:none; }` 규칙이 없어 실제로 숨겨지지 않았음
- 수정
  - 운영 `main.css`에 범용 `.hidden { display: none !important; }` 추가
- 검증
  - 로컬 `http://127.0.0.1:8000/index.html?dev=1&preview=1`에서 편집 패널/분리선 모두 비표시 확인
  - 원격 `https://agenticlab-sh.github.io/skct_tool/index.html?dev=1&preview=1`에서도 편집 패널/분리선 비표시 확인
  - 콘솔 에러 없음 확인

## 스테이징 참고사진 추가 보정
- 사용자 요청
  - 테스트 팝업에서 편집 패널이 화면을 너무 가리므로 접히게 만들기
  - 계산기 우측 연산 기호를 참고사진처럼 `√ / × / - / + / =` 흐름으로 맞추기
  - 계산기 영역 상단에도 `계산기` 제목 표시
  - 스테이징 테스트 사이트만 먼저 수정
- 수정 대상
  - `staging/site/index.html`
  - `staging/site/assets/styles/main.css`
  - `staging/site/assets/scripts/app.bundle.js`
- 구현 내용
  - 팝업 편집 패널에 `펼치기 / 접기` 토글 버튼 추가
  - 실제 팝업 편집 모드에서는 기본값을 `접힘 상태`로 시작하도록 변경
  - 계산기 상단에 `계산기` 라벨 추가
  - 계산기 키 배열을 `C / ← / ÷ / √`, `7 / 8 / 9 / ×`, `4 / 5 / 6 / -`, `1 / 2 / 3 / +`, `0 / 00 / . / =`로 변경
  - `√` 버튼 동작 추가
    - 현재 입력값 기준 제곱근 계산
    - 이력 줄에 `√값 = 결과` 형태로 누적
- 검증
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - 로컬 실제 팝업(`stg_skct_popup_mode`)에서 편집 패널이 접힌 상태로 표시됨 확인
  - 로컬 팝업 화면에서 계산기 라벨이 `계산기`로 보이는 것 확인
  - 계산기 버튼 배열이 `√` 포함 20개 키 순서로 바뀐 것 확인

## 스테이징 분리선 드래그 회귀 수정
- 증상
  - 팝업 편집 모드에서 가로 분리선을 조금만 움직여도 아래쪽이 비정상적으로 늘어나며, 칸 크기 조절이 자연스럽지 않았음
- 원인
  - 드래그 계산은 `timer / utility / calc` 3개 영역 전체 비율을 기준으로 하고 있었는데, 실제 CSS는 `top-bar`와 `tools-layout`이 같은 비율 체계를 쓰지 않고 있었음
  - 즉, 계산식은 3단 분할인데 레이아웃은 `top-bar : tools-layout(고정)` 구조여서 작은 드래그가 과장되어 반영될 수 있었음
- 수정
  - `staging/site/assets/styles/main.css`
  - `.tools-layout`을 고정 `flex: 1`에서 `flex: calc(var(--utils-ratio) + var(--calc-ratio)) 1 0`로 변경
  - 이렇게 바꿔 `timer : utils : calc` 비율 계산과 실제 flex 배치 구조를 일치시킴
- 검증
  - 로컬 실제 팝업에서 상단 분리선 10px 드래그 시 `timer/util/calc` 높이가 정상적으로 재분배되는 것 확인
  - 로컬 실제 팝업에서 중간 분리선 20px 드래그 시 `utility/calc` 높이가 정상적으로 재분배되는 것 확인
  - 아래로 과도하게 확장되는 현상 재현되지 않음

## 계산기 수식 전체 표시 이식
- 사용자 요청
  - `=`를 누르기 전까지 현재 수식이 끊기지 않고 보여야 함
  - 테스트 쪽 계산기 개선본을 운영 서버 계산기에도 반영
  - 계산기 영역만 수정
- 수정 대상
  - `index.html`
  - `main.css`
  - `main.js`
  - `staging/site/assets/styles/main.css`
  - `staging/site/assets/scripts/app.bundle.js`
- 구현 내용
  - 운영 계산기 마크업을 staging 계산기 구조로 교체
    - `계산기` 라벨
    - 계산 기록 3줄 + 현재 입력 1줄 구조
    - `BACK`, `SQRT`, `00` 키 포함
  - 운영 계산기 로직을 staging 기반으로 이식
    - `storedValue / operator / waitingNew / history` 상태 사용
    - `=` 전에는 `왼쪽값 연산자 오른쪽값` 전체 수식이 현재 줄에 유지
    - `=` 후에는 결과가 이력으로 쌓이고 다음 연산으로 이어 계산 가능
  - 수식 잘림 방지 보정
    - 현재 입력 줄은 줄바꿈 가능하게 변경
    - 긴 수식은 길이에 따라 글씨 크기를 자동으로 낮춤
    - 표시용 강제 18자 자르기 로직 제거
- 검증
  - `node --check main.js` 통과
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - 운영 `index.html`에서 `calcHistory`, `BACK`, `SQRT`, `00` 키 마크업 반영 확인
  - 운영 `main.js`에서 구형 `calcDisplay / previous` 참조 제거 확인
  - 원격 `https://agenticlab-sh.github.io/skct_tool/index.html` 응답에서 `calcHistory`, `data-val="SQRT"` 문자열 확인
  - 원격 `https://agenticlab-sh.github.io/skct_tool/main.css` 응답에서 `calc-display-shell`, `calc-line-tight`, `sqrt-btn` 문자열 확인
  - 운영 반영 커밋 `15b14c3`을 `origin/main`에 push 완료

## 계산기 표시 구조 재조정
- 사용자 요청
  - 참고사진처럼 마지막 줄은 항상 현재 값 또는 계산 결과여야 함
  - 이전 값에서 바로 다음 연산으로 이어서 계산할 수 있어야 함
  - 진행 중인 수식은 위쪽 줄에 남고, 마지막 줄은 값 중심으로 보여야 함
- 수정 내용
  - 현재 줄 표시를 `수식 전체`에서 `현재 값/결과값` 표시로 변경
  - 진행 중인 식은 별도 `pending` 줄로 위쪽 이력 영역에 표시
  - 새 연산자를 눌러 연속 계산할 때도 직전 식이 위쪽 이력으로 누적되도록 수정
  - `=` 저장 형식도 `수식 = 결과` 한 줄이 아니라 `위쪽 수식 + 마지막 줄 결과` 구조에 맞게 조정
  - `SQRT`도 계산 결과는 마지막 줄에 두고, 적용식은 위쪽 이력에 남기도록 보정
 - 검증
   - `node --check main.js` 통과
   - `node --check staging/site/assets/scripts/app.bundle.js` 통과
   - 계산 시나리오 확인:
     - `7 + 8 =` -> 위줄 `7 + 8`, 마지막 줄 `15`
     - 이어서 `× 2` 입력 중 -> 위줄 `7 + 8`, `15 × 2`, 마지막 줄 `2`
     - 다시 `=` -> 위줄 `7 + 8`, `15 × 2`, 마지막 줄 `30`

## 계산기 우선순위 평가 전환
- 사용자 요청
  - 연산자 버튼만 눌러서는 바로 계산되지 않아야 함
  - `=`를 눌렀을 때만 계산되어야 함
  - 일반 계산기처럼 `*`, `/` 우선순위를 지켜야 함
  - 운영 배포까지 진행
- 수정 내용
  - 계산 상태를 `storedValue/operator` 방식에서 `expressionTokens` 방식으로 교체
  - `+`, `-`, `×`, `÷` 입력은 식만 누적하고 즉시 계산하지 않도록 변경
  - `=` 입력 시에만 전체 식을 평가하도록 변경
  - 식 평가는 `*`, `/` 선처리 후 `+`, `-`를 처리하는 2단계 계산으로 구현
  - 연속 계산 시에도 결과값에서 새 식을 시작하도록 유지
  - staging 과 운영 스크립트를 같은 규칙으로 동기화

## 테스트 창 우측 버튼 열 및 도구 탭 정리
- 사용자 요청
  - 채팅/물음표 버튼이 차지하는 우측 열을 별도 폭으로 예약하고, 그 폭을 조절하면 버튼 폭도 함께 바뀌게 만들기
  - 우측 열은 먼저 공간을 빼고 남은 영역에서 메모/계산기 본문이 동일하게 줄어들어야 함
  - 메모장/그림판/삭제 UI를 참고 사진처럼 정리
  - 메모 글씨 크기와 그림판 선 굵기 조절은 상단이 아니라 `설정` 버튼 안으로 이동
- 수정 대상
  - `staging/site/index.html`
  - `staging/site/assets/styles/main.css`
  - `staging/site/assets/scripts/app.bundle.js`
  - `admin.html`
- 구현 내용
  - `toolUiConfig.sideButtonColumnRatio` 추가
  - `.tools-layout`에 우측 예약 열 패딩을 부여하고, 예약 열 폭에 따라 채팅/물음표 버튼 크기도 같이 조절되도록 변경
  - 하단 물음표 버튼은 기존 하단 여백 패딩을 반영해 같이 올라가도록 보정
  - 팝업 편집기에 `우측 버튼 열` 슬라이더 추가
  - 개발자 페이지 요약/저장 정규화에도 `sideButtonColumnRatio`를 포함시켜 저장 누락 방지
  - 상단 탭은 평평한 텍스트형으로 바꾸고, `삭제`는 별도 사각 버튼으로 분리
  - 메모장 글씨 크기/그림판 선 굵기 슬라이더는 `설정` 모달 내부 `도구 설정` 섹션으로 이동
- 검증
  - `node --check staging/site/assets/scripts/app.bundle.js` 통과
  - 새 문자열 확인:
    - `popupSideColumnRange`
    - `sideButtonColumnRatio`
    - `--tools-right-rail-button-size`
    - 관리자 요약의 `우측 버튼 열`

## 계산기 클릭 포커스 보정
- 사용자 요청
  - 계산 목록 영역을 클릭했을 때 계산기로 키보드 포커스가 와야 함
- 수정 내용
  - 운영 `main.js`, 스테이징 `staging/site/assets/scripts/app.bundle.js`
  - 계산기 섹션 자체를 포커스 가능한 영역으로 만들고, 계산 목록/빈 영역 클릭 시 계산기 섹션으로 포커스를 이동하도록 변경
  - 메모장에 포커스가 남아 있던 경우 클릭 시 먼저 blur되도록 보정
  - 계산 버튼 자체 클릭은 기존 버튼 동작을 방해하지 않도록 제외
## 운영 OMR 폭 축소 및 팝업 재검증
- 사용자 요청
  - 현재 팝업 창 크기는 유지한 채 OMR 폭을 더 좁혀 메인 도구 영역 여백을 최대한 확보
  - 다만 OMR을 켰을 때 5번 선택지까지 보여야 하며, 최종값은 캡처를 보며 판단
  - 실제 서버에도 반영
- 원인
  - 기존 운영 기본값 `OMR 34%`에서는 팝업 폭 약 400px 기준으로 OMR 자체보다 내부 여백이 커서 5번 선택지가 잘렸음
- 실측 확인
  - Chrome DevTools로 운영 팝업과 로컬 수정본을 14인치에 가까운 창 크기로 캡처
  - `28%` 적용 시 5번이 다시 잘리는 것 확인
  - `30%` 적용 시 5지선다가 모두 보이고, 메모장/계산기 폭도 더 확보되는 것 확인
- 수정 내용
  - `main.css`, `staging/site/assets/styles/main.css`
    - `.omr-body` 우측 패딩 `5px -> 3px`
    - `.subject-group` 패딩/간격 축소
    - `.subject-title` 간격/폰트 축소
    - `.q-num` 폭 `18px -> 14px`
    - `.q-options` 간격 `2px -> 1px`
    - `.q-opt` 크기 `20px -> 15px`
    - `.q-skip-btn`도 함께 축소
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - 팝업 OMR 기본값 `0.34 -> 0.30`
    - 팝업 OMR 최소폭 `130px -> 120px`
    - 팝업 OMR 비율 하한 `0.16 -> 0.12`
  - `admin.html`
    - 관리자 popupLayout 정규화 fallback을 `0.30` 기준으로 보정
- 운영 Firebase 갱신
  - `config/popupLayout.omrWidthRatio = 0.30`
  - `staging_hidden_v1/config/popupLayout.omrWidthRatio = 0.30`
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 팝업 캡처에서 `30%` 상태로 5번까지 모두 노출 확인
## 시작/정지 버튼 아이콘 단순화
- 사용자 요청
  - 시작/정지 버튼을 더 간략하게 보이게 하고 싶음
  - 시작할 때는 시작 기호, 정지할 때는 중지 기호만 보이게 변경
- 수정 내용
  - `index.html`, `staging/site/index.html`
    - 타이머 버튼을 텍스트형에서 아이콘 전용 폭 버튼으로 축소
    - 기본 텍스트를 `▶`로 변경
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - `syncTimerPlayButtonLabel()` helper 추가
    - 정지 상태는 `▶`, 실행 상태는 `■`로 표기
    - 일반 클릭 토글뿐 아니라 타이머 종료, 점수 모드 진입, 중간 정지 경로에서도 helper를 공통 사용
    - 접근성용 `aria-label`, `title`은 각각 `타이머 시작`, `타이머 중지`로 유지
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`

## 숨김 고급 기능 분리 및 타이머/계산기 개선
- 사용자 요청
  - `통합 설정` 연타로만 들어갈 수 있는 숨김 고급 기능 추가
  - 고급 기능 팝업은 비밀번호 목록 인증 후 사용
  - 기본 비밀번호는 `0208`
  - 문항 건너뛰기 시 문항별 남은 시간 즉시 초기화
  - 문항별 상세 통계를 텍스트 파일로 다운로드
  - 계산기 기록은 `Ans = 값`, `수식 = 결과`, 현재 줄 4줄 구조로 일반 계산기처럼 누적
  - 전체 남은 시간이 예정보다 먼저 끝나는 현상 원인 점검
- 수정 내용
  - `index.html`, `staging/site/index.html`
    - `통합 설정` 제목에 숨김 진입용 `settingsTitleTrigger` ID 부여
  - `advanced-tools.html`, `staging/site/advanced-tools.html`
    - 숨김 고급 기능 전용 팝업 신설
    - 비밀번호 입력 후에만 상태 보기, TXT 다운로드, 비밀번호 목록 저장 기능 노출
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - `SKCTAdvancedBridge` 추가
    - 비밀번호 목록 기본값 `0208`, 브라우저 저장소 관리 함수 추가
    - `통합 설정` 제목 7회 연속 클릭 시 고급 기능 팝업 열기
    - 문항 전환 함수를 `recordCurrentQuestionTiming + clearQuestionTools + advanceQuestion` 구조로 통합
    - `문항 건너뛰기` 클릭 시 문항 시간 즉시 기록 후 `questionSpentSec = 0`으로 초기화
    - 상세 통계 HTML 생성 로직을 모델/텍스트 다운로드 함수로 분리
    - 계산기 이력에 `Ans = 결과값` 아카이브와 `수식 = 결과` 기록을 분리
    - 설정 전체 시간과 페이즈 합계가 어긋날 때 조기 종료가 나지 않도록 `max(전체 설정값, 페이즈 합계)` 기준 총시간 적용
  - `docs/agent/36_HIDDEN_ADVANCED_FEATURES.md`
    - 숨김 진입, 인증, 일반 기능과의 경계, 파일 분리 규칙 문서화
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 문자열 확인
    - `settingsTitleTrigger`
    - `SKCTAdvancedBridge`
    - `Ans =`
    - `downloadDetailedStatsText`
- 배포 계획
  - 운영 반영은 보류
  - `staging/site/*`와 문서만 선택 커밋하여 테스트 URL에 먼저 배포

## 운영 반영
- 사용자 승인
  - staging 확인 후 운영 반영 승인
- 운영 반영 파일
  - `index.html`
  - `main.js`
  - `advanced-tools.html`
  - `AGENTS.md`
  - `50_AGENT_LAST_WORK_REPORT.md`
- 운영 코드 커밋
  - `172e5bb` `Deploy hidden advanced tools to production`
- 운영 검증 포인트
  - 운영 `index.html`에 `settingsTitleTrigger` 존재
  - 운영 `main.js`에 `SKCTAdvancedBridge`, `recordCurrentQuestionTiming`, `downloadDetailedStatsText`, `Ans =`, `getEffectiveConfiguredTotalSeconds` 존재
  - 운영 `advanced-tools.html`에 비밀번호 입력과 TXT 다운로드 버튼 문자열 존재

## 쉬는 시간 스킵 및 이전 문항 재선택 허용
- 사용자 요청
  - 쉬는 시간 오버레이에서 다음 과목으로 바로 넘어가는 버튼 추가
  - 실전 모드에서 이전 문항 답안 수정 허용
  - 과거 문항 재수정은 문항별 소요 시간 통계에 영향 없음
- 수정 내용
  - `index.html`, `staging/site/index.html`
    - `omrBreakOverlay` 안에 `쉬는 시간 건너뛰기` 버튼 추가
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - `advancePhaseBoundary()`로 과목 종료/쉬는 시간 종료/쉬는 시간 스킵 로직 통합
    - 쉬는 시간 스킵 시 남은 쉬는 시간만큼 `totalSeconds`도 같이 차감하고 바로 다음 과목 시작
    - OMR 활성 조건을 `현재 문항`에서 `현재+이전 문항`으로 변경
    - 현재 문항 클릭일 때만 자동 다음 문항 이동
    - 이전 문항 클릭은 답만 바꾸고 현재 문항 위치/문항 시간 기록은 그대로 유지
- 로컬 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 문자열 확인
    - `breakSkipBtn`
    - `isAnswerEditableIndex`

## 전체 시간 75분 고정 복원
- 사용자 요청
  - 전체 시간은 75분이어야 함
  - 4분 일찍 끝나던 문제도 함께 해결
- 수정 내용
  - `main.js`, `staging/site/assets/scripts/app.bundle.js`
    - 전체 제한 시간은 다시 `configTotalMins * 60`만 사용
    - 쉬는 시간에는 `totalSeconds`를 차감하지 않도록 변경
    - 따라서 전체 75분은 풀이 시간 합계 기준으로 유지되고, 쉬는 시간 4분은 별도 페이즈 시간으로만 처리
  - `advanced-tools.html`, `staging/site/advanced-tools.html`
    - 상태 문구를 `전체 제한 시간(쉬는 시간 제외)` 기준으로 수정
  - 상세 통계 TXT 상단 문구도 같은 기준으로 수정
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 문자열 확인
    - `전체 제한 시간(쉬는 시간 제외)`
    - `return configTotalMins * 60;`
    - `currentPhase?.type !== 'break'`

## 일반 모드 제거 항목 정리 1차 반영
- 사용자 요청
  - `docs/TODO/TODO1.md` 기준으로 일반 모드에서 제거할 항목을 먼저 스테이징에 반영
  - 다른 기존 기능에 영향이 없는지 먼저 점검
- 반영 파일
  - `staging/site/index.html`
  - `staging/site/assets/scripts/app.bundle.js`
  - `staging/site/assets/styles/main.css`
- 수정 내용
  - 일반 모드
    - 좌측 사이드바의 `무적모드(테스트)` 버튼 제거
    - 우측 채팅/물음표 버튼 열 숨김 및 우측 여백 예약 폭 `0px` 처리
    - 계산기 `√` 버튼 숨김
    - 과목별 상세 통계 직접 버튼 숨김
    - 설정 모달의 `문항별 시간 가이드`, `도구 설정` 섹션 숨김
    - 타이머 시작 후에도 문항별 가이드 타이머가 다시 노출되지 않도록 표시 조건에서 제외
  - 고급 모드
    - `고급버전` 문구를 브라우저 제목과 설정 버전 표기에 반영
    - `고급 기능` 안내 모달에 `우측 버튼 열`, `무적모드`, `상세 통계`, `도구 설정`, `문항별 시간 가이드`, `계산기 루트 버튼` 설명 추가
- 로컬 검증
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 브라우저 `http://127.0.0.1:8000/staging/site/index.html?stage=1`
    - 일반 모드에서 우측 버튼 열 `0px`, 채팅/물음표 숨김, `√` 숨김, 가이드 타이머 숨김 확인
    - 일반 모드 설정에서 `문항별 시간 가이드`, `도구 설정` 섹션 비노출 확인
  - 로컬 브라우저 `http://127.0.0.1:8000/staging/site/index.html?stage=1&advanced=1`
    - 제목 `고급버전` 반영 확인
    - 우측 버튼 열/`√` 버튼 복원 확인
    - 고급 설정에서 `문항별 시간 가이드`, `도구 설정` 섹션 노출 확인
    - `고급 기능` 모달에 새 안내 항목 반영 확인

## 일반 모드 고급 안내 버튼 추가 및 운영 반영
- 사용자 요청
  - 일반 모드에서도 고급 기능이 무엇인지 알려주는 버튼 추가
  - 사용 희망자는 `drgon28@naver.com`으로 문의하도록 안내
  - 스테이징 승인된 일반/고급 분리를 운영 서버에도 이식
- 반영 파일
  - `index.html`
  - `main.js`
  - `main.css`
  - `staging/site/index.html`
  - `staging/site/assets/scripts/app.bundle.js`
  - `staging/site/assets/styles/main.css`
- 수정 내용
  - 일반 모드 사이드바에 `고급 안내` 버튼 추가
  - `advancedGuideModal`을 추가해 고급 기능 목록과 문의 메일 안내 표시
  - 운영 본문에도 스테이징에서 승인된 일반 모드 제거 규칙 이식
    - `무적모드` 버튼 제거
    - 우측 버튼 열/채팅/물음표/`√`/상세 통계 직접 버튼/문항별 시간 가이드/도구 설정을 고급 전용으로 분리
  - Firebase `appName` 로드 시에도 고급 모드 제목이 `고급버전`으로 유지되게 보정
- 로컬 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 브라우저 `http://127.0.0.1:8000/index.html?dev=1`
    - 일반 모드에서 `고급 안내` 버튼 노출 확인
    - `고급 안내` 모달에 `drgon28@naver.com` 문구 확인
    - 일반 모드에서 `√`와 우측 버튼 열 비노출 확인
  - 로컬 브라우저 `http://127.0.0.1:8000/index.html?dev=1&advanced=1`
    - 제목 `고급버전` 확인
    - 고급 모드에서 `고급 안내` 버튼 숨김, `고급 기능` 버튼 노출 확인
    - 우측 버튼 열/`√` 버튼 복원 확인
- 추가 마무리
  - GitHub Pages 캐시로 HTML만 먼저 갱신되고 CSS/JS가 늦게 따라오는 현상을 확인
  - `index.html`, `staging/site/index.html`의 자산 URL에 `v=202604060950` 쿼리를 붙여 캐시 버스팅 적용

## 일반 고급 안내 문구 수정
- 사용자 요청
  - 일반 이용자 페이지의 `고급 기능 안내` 첫 문구를 더 간단한 표현으로 수정
- 수정 내용
  - `index.html`
  - `staging/site/index.html`
  - 일반 안내 모달의 `1. 우측 버튼 열` 설명을 `실제 skct 환경과 유사하게 여백과 버튼들이 제공됩니다`로 변경
  - 고급 이용자 전용 모달 문구는 그대로 유지

## 후원 목록 애니메이션 속도 관리자 조절 추가
- 사용자 요청
  - 후원 목록 애니메이션 속도를 개발자 페이지에서 조절 가능하게 변경
- 반영 파일
  - `admin.html`
  - `index.html`
  - `main.js`
  - `staging/site/admin.html`
  - `staging/site/index.html`
  - `staging/site/assets/scripts/app.bundle.js`
- 수정 내용
  - 관리자 페이지 `후원/운영 문구 관리`에 `후원 목록 전환 속도(초)` 입력칸 추가
  - `supportConfig.sponsorTickerSeconds`를 운영/스테이징 모두 저장/로드하도록 연결
  - 사용자 페이지 후원 ticker의 기존 `4000ms` 하드코드를 제거하고 저장값 기반으로 회전 속도 계산
  - 후원 목록이 다시 렌더링될 때 기존 interval을 정리하도록 `clearInterval` 처리 추가
  - 캐시 버스팅을 위해 `index.html`, `staging/site/index.html`의 스크립트 버전을 `v=202604061035`로 갱신
- 로컬 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 코드 검색으로 `supportTickerSeconds` 저장/로드와 ticker interval 사용처 확인

## 문항 통계 요약 확장 및 건너뜀 오답 처리 설정 추가
- 사용자 요청
  - 통계 요약을 `맞은 / 푼 / 전체`, `정답률(푼 문제 대비)`, `정답률(전체 문제 대비)`, `건너뛴 문제`, `못 푼 문제` 구조로 확장
  - 고급 모드 상세 통계에는 문항 번호, 소요시간, 입력답, 정답, 결과를 체계적으로 표시
  - 일반/고급 설정 모두에서 `건너뛴 문제를 오답 처리` 여부를 선택 가능하게 추가
- 사실 확인
  - `건너뛴 문제는 오답풀이에서 답 입력이 안 된다`는 제보는 로직 버그가 아니었음
  - 기존에도 정답 입력 모드에서는 건너뛴 문제를 다시 눌러 정답 입력이 가능했고, 이번 작업의 핵심은 통계 모델과 안내 체계 보강이었음
- 반영 파일
  - `staging/site/index.html`
  - `staging/site/assets/scripts/app.bundle.js`
  - `index.html`
  - `main.js`
- 수정 내용
  - 일반 요약 패널을 기존 3줄에서 5줄 구조로 확장
  - 상세 통계 텍스트/TXT와 모달에 전체 요약, 과목별 `맞은/푼/전체`, `건너뜀`, `못 풂`, 이중 정답률을 추가
  - 문항별 상세 행에 `입력답`, `정답`, `결과`, `소요시간`을 함께 표시
  - `건너뜀 오답 처리`가 켜져 있으면 건너뛴 문항을 `건너뜀(오답)`으로 집계하고, 꺼져 있으면 `건너뜀`으로 별도 유지
  - 설정 저장 시 `skct_score_cfg`, `stg_skct_score_cfg`로 로컬 저장되게 연결
  - 설정 변경 후 이미 열린 통계 패널/상세 모달도 즉시 다시 계산되도록 재렌더링 추가
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 서버 `http://127.0.0.1:8124/index.html`, `http://127.0.0.1:8124/staging/site/index.html` 응답 `200` 확인
  - 원격 스테이징 응답에서 `statSummary`, `cfgSkippedAsWrong`, `stg_skct_score_cfg`, `건너뜀 오답 처리` 문자열 확인
  - 원격 운영 응답에서 `statSummary`, `cfgSkippedAsWrong`, `skct_score_cfg`, `건너뜀 오답 처리` 문자열 확인
- 반영 커밋
  - 스테이징 코드: `45306c4`
  - 운영 코드: `52ea5cc`

## 공개 문의 이메일 교체
- 사용자 요청
  - 웹에 노출되는 문의 이메일을 `drgon28@naver.com`에서 `zhdlsqpdj@gmail.com`으로 교체
  - 운영 반영까지 수행
- 확인 결과
  - 실제 웹 노출 경로는 `index.html`, `staging/site/index.html` 2곳
  - 공개 README에도 같은 메일이 있어 함께 교체
  - 운영/스테이징 Firebase `supportConfig.contactUrl`은 빈 값이라 런타임에서 구 메일을 다시 노출시키는 원격 설정은 없었음
  - 대신 운영 Firebase `config/notice`, `config/notice_help`, `config/notice_community` 메시지에 구 메일이 남아 있었고, 개발자 공지 노출은 이 값을 읽는 구조였음
  - 해당 3개 경로를 직접 갱신해 공지 문구 메일까지 모두 `zhdlsqpdj@gmail.com`으로 교체
- 반영 파일
  - `index.html`
  - `staging/site/index.html`
  - `README.md`
- 검증
  - `rg -F "drgon28@naver.com"`로 교체 대상 파일 내 잔여 문자열 없음 확인
  - 원격 운영 HTML에서 `zhdlsqpdj@gmail.com` 존재, 구 메일 미존재 확인
  - 원격 스테이징 HTML에서도 동일하게 확인
  - 운영 RTDB `config` 전체 JSON에서 구 메일 미존재, 신 메일 존재 확인
- 반영 커밋
  - 운영 코드: `eb59be3`

## 스테이징 고급 진입 UX 개편 및 고급 구독 관리 표 도입
- 사용자 요청
  - `고급 안내` 버튼 최상단에서 바로 비밀번호를 입력해 고급 버전 팝업을 열 수 있게 변경
  - 기존 `통합 설정 7연타` 진입은 이스터에그로 유지
  - 개발자 페이지의 고급 비밀번호 목록을 표 형태의 `고급 구독 관리`로 개편
  - 앱/링크, 타이머/레이아웃, 후원 문구, 후원 내역, 고급 구독을 각각 저장할 수 있게 버튼 분리
  - 토스 결제 연동은 별도 프로젝트로 진행할 예정이므로, 지금은 설계 문서만 작성
- 설계 문서
  - `docs/2026-04-06_토스_고급구독_분리프로젝트_설계서.md`
- 반영 파일
  - `staging/site/index.html`
  - `staging/site/assets/scripts/app.bundle.js`
  - `staging/site/advanced-tools.html`
  - `staging/site/admin.html`
- 수정 내용
  - `고급 안내` 모달 상단에 `고급 비밀번호 입력 + 고급 모드 열기` 블록 추가
  - 비밀번호 일치 시 바로 고급 버전 팝업을 열고, 기존 이스터에그 진입창도 같은 검증 로직을 쓰도록 정리
  - 스테이징 사용자 앱은 더 이상 평문 비밀번호 목록을 직접 쓰지 않고, `구독 레코드(passwordSalt + passwordHash)`만 읽어 검증
  - 개발자 페이지에 `고급 구독 관리` 표를 추가해 상태, 구독명, 사용자 식별, 새 비밀번호, 만료일, 외부 ID, 메모를 행 단위로 관리
  - 저장 시 평문 비밀번호는 해시로 바꿔 저장하고, 이후엔 해시만 남도록 변경
  - `전체 저장`은 남겨두되, `앱/링크 저장`, `타이머/레이아웃 저장`, `후원 문구 저장`, `후원 내역 저장`, `고급 구독 저장` 버튼을 각각 추가
  - 스테이징 RTDB `staging_hidden_v1/config/advancedFeatureConfig`도 기본 `0208`을 해시 스키마로 마이그레이션
- 검증
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 Playwright 검증
    - `staging/site/index.html?stage=1`에서 `advancedAccessPasswordInput`, `advancedAccessSubmitBtn`, `window.SKCTAdvancedBridge` 존재 확인
    - `window.SKCTAdvancedBridge.validatePassword('0208') === true` 확인
    - `0208` 입력 후 `window.open` 호출 1회, `stg_skct_advanced_unlock` 저장 확인
    - `staging/site/admin.html`에서 구독 표와 각 저장 버튼 DOM 존재, 페이지 에러 없음 확인
  - 원격 스테이징 응답에서 `advancedAccessPasswordInput`, `advancedFeatureSaveBtn`, `generalMetaSaveBtn`, `runtimeDefaultsSaveBtn`, `supportSaveBtn`, `sponsorsSaveBtn`, `validateAdvancedPassword`, `passwordHash` 문자열 확인
- 보안 메모
  - 정적 웹 전체 코드를 완전히 숨기거나 복제를 원천 차단하는 것은 불가능함
  - 이번 스테이징 작업에서는 실제 효과가 있는 보안 조치로 `평문 비밀번호 제거`와 `해시 기반 검증`까지만 적용
- 반영 커밋
  - 스테이징 코드: `4560507`

## 스테이징 고급 진입 흐름 추가 보강
- 사용자 요청
  - “스테이징 구조를 다듬는다”는 표현을 실제로 체감되는 UX/운영성 보강으로 알아서 진행
- 반영 파일
  - `staging/site/index.html`
  - `staging/site/assets/scripts/app.bundle.js`
  - `staging/site/advanced-tools.html`
  - `staging/site/admin.html`
- 수정 내용
  - 일반 화면의 `고급 안내` 모달에 `구독 상태 요약`, `비밀번호 보기`, `인증된 브라우저면 바로 열기`를 추가
  - 원격 고급 구독 설정이 아직 안 내려왔을 때는 버튼을 비활성화하고, 준비 완료 후에만 입력 검증이 되도록 보강
  - 비밀번호 오입력 누적 시 짧은 재시도 대기시간을 두어 무차별 입력을 조금 더 어렵게 조정
  - 이스터에그 진입용 `advanced-tools.html`도 원본 창의 구독 설정 로딩 완료를 기다린 뒤 검증하게 보강
  - 개발자 페이지 `고급 구독 관리`에 활성/일시중지/만료/7일 내 만료 요약 카드 추가
  - 각 구독 행에 `6자리 생성` 버튼을 넣어 임시 비밀번호 생성 편의 제공
  - 고급 구독 저장 시 `새 비밀번호 중복`, `외부 ID 중복`을 막고, 저장 결과를 표 하단 상태 문구로 바로 보여주도록 보강
  - `전체 저장` 도중 고급 구독 저장이 막히면 전체 저장도 중단되고 이유를 토스트로 안내하도록 정리
- 검증
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - `node --check` 방식으로 `staging/site/admin.html`, `staging/site/advanced-tools.html` 인라인 스크립트 문법 확인
  - 로컬 브라우저 검증
    - `고급 안내` 모달에서 `0208` 입력 후 고급 팝업 열기 상태 문구 확인
    - `비밀번호 보기` 토글로 입력 타입이 `text`로 바뀌는지 확인
    - 인증 후 `인증된 브라우저면 바로 열기`로 재오픈 가능한지 확인
    - 요약 문구가 `사용 가능한 고급 이용권 1건`, 이후 `이미 인증된 브라우저` 상태로 바뀌는지 확인
- 메모
  - 이번 턴도 스테이징만 수정했고 운영 파일은 건드리지 않음

## docs 체계 재정리
- 사용자 요청
  - 지금까지 해온 일을 `docs/` 아래에 체계적으로 정리
- 새 기준 문서
  - `docs/README.md`
  - `docs/SKCT_TOOL_전체구조_및_운영기준.md`
  - `docs/SKCT_TOOL_기능_변경이력_요약.md`
- 수정 문서
  - `docs/SKCT_TOOL_운영가이드.md`
- 정리 원칙
  - 기존 작업 로그는 그대로 유지
  - 새 문서는 `인덱스`, `현재 구조`, `변경 이력` 역할로 분리
  - 오래된 운영가이드에는 최신 기준 문서로 가는 링크를 앞부분에 추가
- 기대 효과
  - 새 작업자가 `어디를 봐야 하는지` 바로 찾을 수 있음
  - 개별 작업 로그와 현재 기준 문서를 구분해 읽을 수 있음

## 운영 반영: 고급 진입/구독 관리 개선
- 사용자 승인 후 운영 반영 범위
  - `index.html`
  - `main.js`
  - `advanced-tools.html`
  - `admin.html`
- 운영 반영 내용
  - 일반 화면 `고급 안내` 모달에서 바로 비밀번호 입력 후 고급 팝업을 열 수 있게 이식
  - `비밀번호 보기`, `인증된 브라우저면 바로 열기`, 이용권 상태 요약, 오입력 대기시간 보강 이식
  - 운영 관리자 페이지도 `고급 구독 관리` 표와 개별 저장 버튼 구조로 전환
  - 운영 RTDB `config/advancedFeatureConfig`를 평문 비밀번호 목록에서 해시 기반 `subscriptions` 구조로 직접 마이그레이션
- 운영 RTDB 마이그레이션 결과
  - `0208`
  - `dgd0392`
  - 두 값 모두 `passwordSalt + passwordHash` 구조로 저장됨
- 검증
  - `node --check main.js`
  - `admin.html`, `advanced-tools.html` 인라인 스크립트 문법 확인
  - 로컬 운영 페이지에서 `고급 안내` 모달 오픈 후 `현재 사용 가능한 고급 이용권 2건` 표시 확인
  - `0208` 입력 시 고급 팝업 열기 상태, 재열기 버튼 동작 확인
- 반영 커밋
  - 운영 코드: `0b02d9f`

## 운영 반영: 고급 비밀번호 오류 문구 정정
- 사용자 문의
  - `manual-seed-dgd0392`가 비밀번호처럼 보이고 `dgd0392`를 넣으면 만료된 것으로 오해된다는 제보
- 확인 결과
  - `manual-seed-dgd0392`는 비밀번호가 아니라 운영 구독 레코드 식별용 `externalId`
  - 실제 비밀번호 `dgd0392`는 운영 RTDB에 활성 상태로 저장되어 있었고, 운영 실페이지에서도 인증 성공 확인
  - 문제 원인은 `비밀번호 불일치`와 `만료/비활성`을 구분하지 않는 공용 오류 문구였음
- 수정 내용
  - 운영/스테이징 공통 검증 로직에 상세 결과(`invalid`, `expired`) 반환 추가
  - 일반 `고급 안내` 모달, 숨김 `advanced-tools.html` 모두 오류 문구 분리
  - 운영 `index.html`, 스테이징 `staging/site/index.html`의 JS 버전을 `v2026.04.06.1600`으로 갱신해 캐시 회피 강화
- 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - `advanced-tools.html` 인라인 스크립트 문법 확인
  - 운영 실페이지에서 `dgd0392` 입력 후 고급 팝업 열기 확인
- 반영 커밋
  - 운영 코드: `5ff5978`

## 스테이징 반영: 고급모드 타이머 과목 건너뛰기
- 사용자 요청
  - 고급모드 타이머에 `과목 건너뛰기` 기능 추가
  - 누르면 다음 과목으로 바로 전환되어 시작되고, 전체 남은 시간과 OMR도 그 단계에 맞게 함께 이동해야 함
- 스테이징 수정 범위
  - `staging/site/index.html`
  - `staging/site/assets/styles/main.css`
  - `staging/site/assets/scripts/app.bundle.js`
- 구현 내용
  - 타이머 우측에 고급모드 전용 `다음 과목` 버튼 추가
  - 실전 모드 + 타이머 실행 중 + 다음 과목이 남아 있을 때만 활성화
  - 클릭 시 현재 과목의 남은 시간을 `totalSeconds`에서 즉시 차감
  - 중간 쉬는 시간은 생략하고 다음 과목으로 바로 진입
  - 다음 과목 첫 문항으로 OMR 현재 위치를 이동하고, 이전 과목은 잠금 처리
  - 과목 전환 직후 가이드 타이머가 이전 문항 시간을 이어받지 않도록 `questionSpentSec` 초기화
  - 타이머 시작/정지 직후 버튼 활성화 상태가 바로 바뀌도록 `updateTimerUI()` 동기화 추가
- 로컬 검증
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 서버 `http://127.0.0.1:8131/staging/site/index.html?stage=1&preview=1&advanced=1`에서 고급모드 진입 확인
  - 타이머 시작 후 `다음 과목` 버튼 즉시 활성화 확인
  - `다음 과목` 클릭 시 `75:00 -> 60:00`, `1과목 언어이해 -> 2과목 자료해석` 전환 확인
  - OMR 현재 문항이 `2과목 1번`으로 이동하고 `1과목`이 잠금 처리되는 것 확인
  - 가이드 타이머가 새 과목 기준으로 다시 시작되는 것 확인
- 반영 상태
  - 운영 미반영

## 2026-04-07 스테이징/운영 반영: 고급모드 타이머 초기화
- 사용자 요청
  - 고급모드에만 `과목 초기화`, `전체 초기화`를 추가
  - 스테이징에서 테스트 후 운영까지 반영
- 반영 파일
  - `staging/site/index.html`, `staging/site/assets/styles/main.css`, `staging/site/assets/scripts/app.bundle.js`
  - `index.html`, `main.css`, `main.js`
- 구현 내용
  - 고급모드 타이머 바에 `과목 초기화`, `전체 초기화` 버튼 추가
  - `과목 초기화`
    - 현재 과목 진행 중일 때만 활성화
    - 타이머를 멈추고 현재 과목 시간을 `configSubjectMins` 기준으로 복원
    - 전체 남은 시간도 `현재 과목 전체 시간 + 이후 과목 전체 시간` 기준으로 다시 계산
    - 현재 과목의 문항별 시간 기록만 지우고, 현재 과목 첫 문항으로 OMR 위치 이동
  - `전체 초기화`
    - 타이머를 멈추고 `75분 / 1과목 / 15:00` 기준으로 전체 단계 재구성
    - 모든 문항 시간 기록과 과목 잠금을 초기화하고, OMR 위치를 1번 문항으로 복귀
  - 두 버튼 모두 답안/정답 자체는 지우지 않음
  - 캐시 우회를 위해 운영/스테이징 자산 버전을 `v202604071015`로 갱신
- 로컬 검증
  - `node --check main.js`
  - `node --check staging/site/assets/scripts/app.bundle.js`
  - 로컬 서버 `http://127.0.0.1:8132/staging/site/index.html?stage=1&preview=1&advanced=1`에서 고급모드 진입 후 실클릭 검증
  - `과목 초기화` 검증
    - `74:49 / 14:49` 상태에서 클릭 후 `75:00 / 15:00`, `1과목 언어이해`, `1번 문항` 복귀 확인
  - `전체 초기화` 검증
    - `다음 과목`으로 `2과목 자료해석 / 59:57` 상태 이동 후 클릭
    - `75:00 / 15:00`, `1과목 언어이해`, `1번 문항`, 타이머 정지 상태 복귀 확인
- 반영 커밋
  - 운영/스테이징 코드: `fbdbb62`

## 2026-04-07 스테이징 반영: 고급모드 상단 타이머/좌측 패널 디자인 압축
- 사용자 요청
  - 고급모드 타이머 초기화 버튼을 더 압축적으로 사용
  - 재생 버튼이 밀리지 않게 상단 타이머 레이아웃 재배치
  - 상단 타이머 구역을 조금 더 넓게 쓰고, 좌측 패널 버튼도 최적화
- 반영 파일
  - `staging/site/index.html`
  - `staging/site/assets/styles/main.css`
- 구현 내용
  - 좌측 사이드 버튼에 공통 `sidebar-shortcut` 폼팩터 적용
  - 버튼 폭/높이/간격/구분선 구조를 정리해 세로 밀도 개선
  - 상단 타이머 바를 `advanced-timer-shell` 구조로 재배치
  - 재생 버튼은 고정 정사각형 버튼으로 분리하고, 고급 버튼은 `다음 / 과↺ / 전↺` 압축 라벨로 축소
  - 타이머 요약 영역(`전체 남은 시간`, `현재 과목`) 폭을 확보해 버튼에 밀리지 않도록 조정
  - 스테이징 CSS 자산 버전을 `v202604071120`으로 갱신
- 로컬 검증
  - 로컬 서버 `http://127.0.0.1:8133/staging/site/index.html?stage=1&preview=1&advanced=1`에서 고급모드 확인
  - 버튼 노출 상태
    - `▶ / 다음 / 과↺ / 전↺`가 한 줄에 유지되고 재생 버튼이 밀리지 않는 것 확인
  - 좌측 패널
    - `GUIDE / 연습용 OMR / 설정 / 고급 기능 / 화면 더 줄이기 / 접속자 수 / 게시판 / 커피 후원`이 더 조밀한 동일 폼팩터로 정렬된 것 확인
  - 스크린샷
    - `logs/staging_advanced_timer_sidebar_compact.png`
- 반영 상태
  - 운영 미반영

## 2026-04-07 운영 이식: 고급모드 타이머/좌측 패널 디자인 압축
- 사용자 승인 후 운영 `index.html`, `main.css`, `main.js`에 스테이징 고급모드 레이아웃 이식
- 운영 반영 내용
  - 좌측 패널 버튼을 공통 `sidebar-shortcut` 폼팩터로 통일
  - 상단 타이머를 `advanced-timer-shell` 구조로 재배치
  - 재생 버튼이 밀리지 않도록 고정 버튼화, 고급 버튼 라벨을 `다음 / 과↺ / 전↺`로 압축
  - 운영에는 빠져 있던 `다음 과목` 동작도 함께 복원
  - 일반모드 `고급 기능 안내`와 고급모드 `고급 기능` 설명에 `타이머 초기화` 항목 추가
  - 운영 CSS/JS 자산 버전을 `v202604071145`로 갱신
- 운영 검증
  - `node --check main.js`
  - `main.js`에 `canSkipToNextSubject`, `updateSubjectSkipButton`, `skipCurrentSubjectToNext` 존재 확인
  - `index.html` 설명 목록에 `4. 타이머 초기화` 반영 확인
- 반영 커밋
  - 운영 코드: `aef45ab`

## 2026-04-08 경남 창원시 성산구 기준 초기 비용 정리 및 저부하 접속자 집계 설계 문서화
- 사용자 요청
  - 경남 창원시 성산구 기준으로 사업자등록/통신판매업 신고 진행 시 실제 초기 비용 계산
  - Firebase에 무리를 주지 않는 방식으로 동시 접속자 누적 기록이 가능한지 검토하고 문서화
- 비용 정리
  - 사업자등록 자체는 별도 수수료가 없는 것으로 봄
  - 창원시는 인구 50만 이상 시에 해당하고, 통신판매업은 등록면허세 면허분 3종 기준을 적용
  - 성산구는 `구 동지역`이므로 `40,500원`으로 보는 것이 맞음
  - 즉, 사용자가 온라인 판매를 위한 최소 공식 행정비용만 본다면 현재 기준 `40,500원`으로 정리
- 반영 파일
  - `docs/2026-04-08_저부하_접속자_집계_설계.md`
  - `docs/README.md`
  - `docs/agent/35_LEARNING_NOTES.md`
- 접속자 집계 설계 요약
  - 현재 구조는 `active_visitors.json` 전체를 반복 fetch하는 읽기 부담이 큼
  - 개선안은 `raw heartbeat 유지 + 1분 집계 + 5분 rollup + 화면은 집계값만 읽기`
  - 현재 프로젝트에는 `Cloud Function 또는 서버리스 cron` 집계 작업자를 두는 방식이 가장 현실적이라고 판단
- 기대 효과
  - 지금 당장 코드를 바꾸지 않아도 “무슨 방식으로 가볍게 바꿀지” 기준이 생김
  - 이후 승인 시 실제 구현을 안전하게 진행할 수 있음

## 2026-04-08 저부하 접속자 집계 1차 구현
- 사용자 요청
  - 최신 코드 기준으로 저부하 접속자 집계 설계를 다시 검토한 뒤 실제 1차 구현 수행
- 최신 코드 확인 결과
  - 운영 `index.html`과 스테이징 `staging/site/index.html`은 `active_visitors.json` 전체를 10초마다 REST fetch
  - 관리자 `admin.html`도 같은 방식으로 10초 polling
  - heartbeat는 30초마다 `active_visitors/{sessionId}`에 타임스탬프를 다시 쓰는 구조였음
- 이번 1차 구현 방향
  - 집계 작업자(Cloud Function / cron) 도입 전, 가장 무거운 반복 REST polling부터 제거
  - Firebase `onValue()` 실시간 구독으로 전환
  - heartbeat 주기는 `30초 -> 60초`로 완화
  - 연결 시점에만 `stats/live_peak_daily/{YYYY-MM-DD}`를 transaction으로 갱신해, 동시 접속 피크 기록의 기초 데이터도 남기도록 보강
- 반영 파일
  - `index.html`
  - `staging/site/index.html`
  - `admin.html`
  - `docs/agent/35_LEARNING_NOTES.md`
- 구현 내용
  - 사용자 통계 모달과 관리자 대시보드의 현재 접속자 집계를 REST polling에서 실시간 구독으로 변경
  - stale threshold를 기준으로 현재 유효 세션만 카운트하도록 공통 함수 추가
  - 사용자/스테이징 presence heartbeat를 `serverTimestamp()` + 60초 주기로 변경
  - 통계 모달 문구를 `10초 주기 업데이트`에서 `실시간 반영`으로 수정
- 검증
  - 로컬 서버 `http://127.0.0.1:8126/index.html` 응답 200 확인
  - 브라우저 자동화 도구는 현재 CDP 연결이 열려 있지 않아 실제 브라우저 실클릭 검증은 미수행
  - diff 기준으로 polling fetch 제거와 구독 전환 반영 확인
- 반영 상태
  - 아직 로컬 작업 상태
  - 운영 / 스테이징 배포 전, 사용자 승인 필요

## 2026-04-08 고급 안내 로그인/구독 신청 흐름 단순화 및 운영 반영 준비
- 사용자 요청
  - 고급 모드 기능의 장점이 더 명확하게 드러나도록 안내 문구 개선
  - 로그인 영역의 불필요한 구조 제거
  - 이용권 신청 흐름을 순서형으로 단순화
  - 신청 조회를 `신청번호 + 조회 비밀번호`가 아니라 `ID + 비밀번호` 기준으로 변경
- 반영 파일
  - `index.html`
  - `main.js`
  - `admin.html`
- 구현 내용
  - `고급 이용 안내` 최상단을 `고급 모드 로그인`으로 단순화하고, `인증된 브라우저면 바로 열기`와 비밀번호 보기 같은 불필요한 보조 UI 제거
  - `고급 모드에서 달라지는 점` 문구를 “어떤 시간이 줄고 무엇이 편해지는지” 중심으로 재작성
  - 이용권 선택/신청 안내를 `금액 확인 -> 투네이션 후원 -> 신청 저장 -> 같은 ID+비밀번호로 확인` 흐름으로 정리
  - 신청서의 메모 항목 제거
  - `조회 비밀번호` 표현을 없애고, 사용자가 직접 정한 비밀번호를 신청/조회/승인 기본값에 함께 쓰도록 수정
  - 신청 조회는 `desiredLoginIdKey` 기준 검색 후 입력 비밀번호로 복호화하는 방식으로 변경
- 검증
  - `node --check main.js`
  - 로컬 정적 서버 `http://127.0.0.1:8127/index.html` 응답 200 확인
  - 로컬 HTML 기준 `고급 모드 로그인`, `신청한 ID`, `사용할 비밀번호`, `신청한 비밀번호` 노출 확인
  - 로컬 HTML 기준 `신청번호`, `조회 비밀번호` 문구 제거 확인
- 반영 상태
  - 운영 반영 직전 로컬 검증 완료

## 2026-04-08 고급 안내 비밀번호 설명 문구 일치화
- 사용자 요청
  - 고급 안내의 신청/조회/로그인 비밀번호 설명이 서로 다르게 보이는 문제 수정
- 반영 파일
  - `index.html`
  - `admin.html`
- 구현 내용
  - 사용자 안내에서 `같은 ID + 비밀번호`가 신청 저장, 상태 조회, 고급 모드 로그인까지 이어진다는 점을 직접적으로 명시
  - 관리자 안내의 오래된 `신청번호 + 조회 비밀번호` 문구를 `신청한 ID + 비밀번호` 기준으로 교체
- 검증
  - 로컬 HTML 기준 `신청번호`, `조회 비밀번호` 미노출 재확인
  - 운영 반영 후 원격 HTML에서도 같은 기준 문구 확인 예정

## 2026-04-08 고급 안내 최신 자산 재로딩 보강
- 사용자 요청
  - 운영에서 예전 고급 안내 문구가 계속 보이는 현상 재점검
- 확인 결과
  - 운영 `index.html` 본문과 `config/siteTextConfig`에는 예전 문구가 남아 있지 않았음
  - 브라우저 캐시로 이전 자산을 보는 가능성이 높다고 판단
- 반영 파일
  - `index.html`
- 구현 내용
  - 신청 조회 설명을 `같은 ID + 비밀번호` 기준으로 더 직접적으로 재서술
  - `subscription-crypto.js`, `main.js` 쿼리 버전을 갱신해 최신 자산 재로딩을 유도

## 2026-04-08 수동 구독 신청 운영 설정 보강 및 실제 동작 복구
- 사용자 요청
  - 신청 저장 시 `운영 설정이 아직 준비되지 않았습니다` 오류가 뜨는 문제 해결
  - 개발자 페이지의 빠진 운영 준비 흐름 보강
- 원인
  - 운영 `config/manualSubscriptionConfig.adminPublicKeyPem`이 비어 있어 사용자 신청 저장이 의도적으로 차단되고 있었음
  - 신청 조회는 `desiredLoginIdKey` 쿼리 인덱스 상태에 따라 실패할 수 있는 구조였음
- 반영 파일
  - `main.js`
  - `admin.html`
- 구현 내용
  - 개발자 페이지 신청 설정 저장 시 공개키가 없으면 새 신청 암호화 키를 자동 생성하도록 보강
  - 개발자 페이지에 `개인키 다운로드` 버튼 추가
  - 개발자 페이지 상태 문구를 “현재 운영 신청 저장이 왜 막히는지” 드러나게 수정
  - 신청 조회는 RTDB 쿼리가 실패하면 전체 신청 목록을 읽어 클라이언트에서 ID 기준 필터링하는 fallback 추가
  - 운영 RTDB `config/manualSubscriptionConfig.adminPublicKeyPem`에 실제 공개키 저장
  - 사용자 페이지 `main.js` 자산 버전을 갱신해 최신 신청 로직을 바로 받도록 조정
- 운영 보조 산출물
  - 관리자 개인키 로컬 백업: `C:\\Users\\kshcg\\.codex\\private\\skct_tool\\manual_subscription_private_20260408_1500.pem`

## 2026-04-08 고급모드 직접 URL 진입 차단 보강
- 사용자 요청
  - `?advanced=1` 직접 링크만으로도 고급모드가 열리는지 점검 및 차단
- 확인 결과
  - 실제 고급 기능 판정은 `advanced=1`만이 아니라 `skct_advanced_unlock`까지 필요했지만, 쿼리 파라미터가 먼저 반영되며 고급처럼 보이는 흔적이 남을 여지가 있었음
- 반영 파일
  - `index.html`
  - `main.js`
- 구현 내용
  - 런타임 플래그 초기화 단계에서 유효한 `skct_advanced_unlock`이 없으면 `advanced=1`을 즉시 무효화
  - 인증 없이 직접 URL로 들어온 경우 `history.replaceState`로 주소에서도 `advanced` 파라미터 제거
  - `main.js`에서도 한 번 더 방어적으로 동일 정리 수행
  - 최신 차단 로직을 바로 받도록 `main.js` 자산 버전 갱신

## 2026-04-08 고급모드 사이드바 표시 조정
- 사용자 요청
  - 고급모드에서는 시각적으로 이미 고급모드라는 점이 드러나도록, 고급 버튼의 자물쇠 아이콘 제거
- 반영 파일
  - `index.html`
  - `main.css`
- 구현 내용
  - 고급모드 전용 사이드바 버튼의 자물쇠 아이콘 제거
  - 버튼 라벨을 `고급 모드`로 바꾸고 텍스트 비중을 조금 높여 현재 상태가 더 바로 보이게 조정

## 2026-04-08 당일 작업 9문장 요약 문서화
- 사용자 요청
  - 오늘 수행한 작업을 9문장으로 요약해 `docs/agent` 폴더에 기록하고 GitHub까지 반영
- 반영 파일
  - `docs/agent/2026-04-08_daily_summary.md`
- 구현 내용
  - 고급 안내/수동 구독/고급 URL 차단/고급모드 시각 표현/.codex 환경 정리까지 포함한 당일 작업을 9문장으로 압축 정리
