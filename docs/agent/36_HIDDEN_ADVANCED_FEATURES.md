...
## 1. 진입 원칙
- 일반 화면에는 `🔒 고급 기능`만 보여주고, 실제 고급 기능은 승인된 라이선스가 확인된 뒤에만 열립니다.
- `📘 가이드`는 일반 모드 사용법 중심으로만 유지하고, 고급 전용 설명은 여기와 `고급 활용` 쪽으로 분리합니다.
...
- 이 상태 바는 최소한 아래 값을 바로 보여줘야 합니다.
  ...
  - 자료 보관함 사용 가능 여부
  - 우측 실제환경 여백 복원 여부
- OMR 하단에는 `고급 버튼 사용 순서` 패널이 나타나야 하며, 정답 입력 -> 채점/통계 -> 반복 연습 순서를 초보자 기준으로 설명해야 합니다.
...
### 2.4 가이드와 고급 기능의 역할 분리
- 일반 `가이드` 모달은 일반 모드 사용 흐름만 설명합니다.
- 일반 `가이드` 모달에는 고급 전용 설명을 길게 넣지 않고, `고급 기능` 버튼으로 보내는 짧은 안내만 둡니다.
- 고급 모드에서 여는 `고급 활용` 모달에만 시각 가이드 이미지와 고급 전용 사용 흐름을 둡니다.
- 이때 사용자는 아래 두 질문에 바로 답을 얻어야 합니다.
  - 일반 모드만 쓸 때는 어디를 보면 되는가
  - 고급 모드에서는 무엇이 추가되는가

### 2.5 고급 활용 시각 가이드
- 고급 활용 모달에는 단순 스크린샷이 아니라, 사용자가 어디를 봐야 하는지 바로 이해할 수 있도록 강조 박스와 라벨이 들어간 이미지를 씁니다.
- 이번 기준 자산은 아래 2개입니다.
  - `images/advanced_mode_status_highlight.png`
  - `images/advanced_mode_omr_highlight.png`
- 강조선과 라벨은 텍스트보다 튀지 않게 유지하고, 선 굵기와 라벨 무게가 과하면 다시 줄이는 것이 원칙입니다.
...
- 일반 `가이드` 모달에 고급 이미지가 들어가지 않는지
- 일반 `가이드` 모달의 고급 기능 안내가 짧은 연결 문구로만 남아 있는지
- 고급 모드 `더보기`에 자료 보관함 버튼이 보이는지
- 고급 모드에서 우측 버튼 자리와 우측 reserve 여백이 함께 복원되는지
- 고급 모드 상태 바에 로그인/만료/자료 보관함/우측 여백 값이 정상 노출되는지
- OMR 하단 `고급 버튼 사용 순서` 패널이 고급 모드에서만 보이는지
- 고급 활용 모달의 이미지 2장이 실제로 로드되는지
