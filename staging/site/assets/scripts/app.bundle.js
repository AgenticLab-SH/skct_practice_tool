...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderBulkColumnOptions = (parsed, selectedQuestionCol, selectedAnswerCol) => {
        ...
        const optionHtml = parsed.columnLabels.map((label, index) => `<option value="${index}">${escapeHtml(label || `열 ${index + 1}`)} (${index + 1})</option>`).join('');
        bulkCorrectQuestionCol.innerHTML = optionHtml;
        bulkCorrectAnswerCol.innerHTML = optionHtml;
    ...
    const buildBulkCorrectPreview = (parsed, questionCol, answerCol) => {
        ...
        parsed.rows.forEach((row) => {
            ...
            if (!Number.isInteger(questionNo) || questionNo < 1 || questionNo > getTotalQuestionCount() || answer == null) {
                invalidRowCount += 1;
                return;
            }
            if (mappedByQuestion.has(questionNo)) duplicateCount += 1;
            mappedByQuestion.set(questionNo, {
                questionNo,
                answer,
        ...
        const mappedRows = [...mappedByQuestion.values()].sort((a, b) => a.questionNo - b.questionNo);
        return {
            mappedRows,
            invalidRowCount,
            duplicateCount
    ...
    const renderBulkCorrectPreview = () => {
        ...
        const questionCol = parseInt(bulkCorrectQuestionCol?.value || '0', 10);
        const answerCol = parseInt(bulkCorrectAnswerCol?.value || '1', 10);
    ...
    const savedScoreCfg = isAdminPreviewMode ? null : JSON.parse(localStorage.getItem('stg_skct_score_cfg'));
    ...
    if (settingsApplyBtn) {
        settingsApplyBtn.addEventListener('click', () => {
            ...
            if (!isAdminPreviewMode) {
                localStorage.setItem('stg_skct_score_cfg', JSON.stringify({ treatSkippedAsWrong: configTreatSkippedAsWrong }));
            }
